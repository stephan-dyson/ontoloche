...
-- schema uses, chosen because it sorts correctly as text. The edge cursor is a keyset
-- on (created_at, edge_id), so that property is what makes `find_edges` pageable.
