# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit open_ontology/contract/test_c19_actions.py and run:
#
...
from open_ontology.aio.adapter import ACTION_CAPABILITY_FLAGS, CAPABILITY_FLAGS, Capabilities
from open_ontology.edges import InstanceRef, TypeRef
from open_ontology.errors import NotSupported, UnknownType
from open_ontology.policy import TierOrder
from open_ontology.types import Evidence, Proposal, Refusal, TypeEntry
from open_ontology.aio.contract._support import action_family, edge_family, seed
...
async def test_c19_44_every_declaration_rule_binds_at_all_three_doors(adapter, make_registry):
    ...
    **The third door is the one round 1 walked through.** ACTIONS.md said *"at
    ...
    that amends one.
    """
    from open_ontology.aio.adapter import ProposalRecord
...
@NEEDS_INVOCATIONS
async def test_c19_62_a_bounded_compensation_lookup_says_so_rather_than_answering_none(
    adapter, make_registry
):
    ...
    **The first cut discarded the page's `complete=False` and returned a bare `None`.**
    ...
    """
    from open_ontology.aio.registry import _LEDGER_LIMIT
    ...
    found, why = await registry._compensated_by(original.invocation_id, "default")
    assert found == compensator.invocation_id and why is None
    back = [
        i for i in (await registry.invocations()).invocations
    ...
    padded, _ = await registry._compensated_by("an-id-no-row-carries", "default")
    assert padded is None
    bounded, sentence = await registry._compensated_by(
...
async def test_c19_63_a_payload_schema_is_keyed_apart_from_the_familys_own_eight_keys(
    ...
    *"not declared in the schema"*. *The family became undeclarable by the act of
    ...
    """
    from open_ontology.actions import ACTION_PAYLOAD_KIND
