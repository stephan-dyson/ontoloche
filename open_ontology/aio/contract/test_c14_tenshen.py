# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit open_ontology/contract/test_c14_tenshen.py and run:
#
...
import open_ontology
from open_ontology import registry as registry_module
...
async def test_c14_01_contortion_1_edge_shape_lives_unvalidated_in_attributes(registry):
...
async def test_c14_02_contortion_2_a_bare_counter_leaves_orphaned_unknowable(
    ...
    tenshen = await make_registry(AsyncDegradedAdapter(adapter, timestamps_usage=False, why=NO_TIMESTAMPS))
    report = await tenshen.usage("blocks")
...
async def test_c14_03_contortion_3_no_status_column_means_everything_migrates_active(registry):
    ...
    entries = await registry.import_types(rows, system="tenshen", imported_by="import:tenshen")
    assert len(entries) == 5
    assert {e.status for e in entries} == {"active"}

    for entry in entries:
        history = (await registry.provenance(entry.name)).history
        assert [e.event for e in history] == ["imported"], (
            "no retirement history is invented for a source that never had one"
        )

async def test_c14_04_contortion_4_auto_approval_becomes_legible_and_enumerable(
    adapter, make_registry
):
    ...
    machine_made = await registry.list_types(created_by="ai")
...
async def test_c14_05_contortion_5_a_discarded_fit_score_leaves_the_evidence_slot_empty(
    adapter, make_registry
):
    ...
    assert (await registry.provenance("supersedes")).evidence == ()

async def test_c14_06_contortion_6_zero_registered_consumers_is_a_reported_null_result(registry):
    ...
    await seed(registry, "blocks", kind="edge", definition="this work item blocks that one")
    report = await registry.consumers("blocks")
...
def test_c14_07_contortion_7_the_package_ships_no_default_type():
    ...
    """
    assert not hasattr(open_ontology, "default_type")
    assert not hasattr(open_ontology, "DEFAULT_TYPE")
    assert "default_type" not in dir(open_ontology)

    public = [
        Path(open_ontology.__file__),
        Path(registry_module.__file__),
        Path(open_ontology.__file__).parent / "types.py",
        Path(open_ontology.__file__).parent / "policy.py",
        Path(open_ontology.__file__).parent / "adapter.py",
    ]
    for path in public:
