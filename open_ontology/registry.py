...
class Registry:
        ...
        if best_score is not None and best_score >= policy.existing_threshold and best_score >= min_confidence:
            ...
            if matched is not None and written is None:
                page = self.adapter.find_types(
                    TypeQuery(
                        namespace=namespace,
                        name_in=self._word_spellings(namespace, candidate)[0],
                        include_retired=True,
                    )
                )
                written = next(
    ...
    def _write_approved(
        ...
        *,
        ...
        holder, holder_why = self._alias_holder(rec.namespace, rec.name, rec.kind)
...
# **`knowable` folds in the READ's own verdict, not only the capability flag** (row
    ...
    def reinstate(
        ...
        **Ruling R19: this covers edge FAMILIES and never edge instances.** An edge
        ...
        if dormant:
            ...
            collision, collision_why = self._alias_clash(
    ...
    def import_types(
        ...
        *,
        ...
        for row in rows:
            ...
            if standing is None:
                holder, holder_why = self._alias_holder(
            ...
            if incoming:
                ...
                if breach is not None:
                    ...
                    continue
                clash, clash_why = self._alias_clash(
                    namespace, name, row.get("kind", kind), incoming
                )
    ...
    def _alias_identity_breach(
        ...
        **The diagnosis is the third trip's, widened once more:** a guard written for one
        ...
        for alias in aliases:
            ...
            after: str | None = None
            cursors: set[str] = set()
            spellings, partial_why = self._word_spellings(namespace, alias)
            while True:
                page = self.adapter.find_types(
                    TypeQuery(
                        namespace=namespace,
    ...
    def _declared_predicate_moved(self, namespace: str, declared: str) -> str | None:
        ...
        **Two ways a word moves, and both are checked.** The tombstone with a live
        ...
        if rec is not None:
            ...
            # already uses it.
            closure, _complete, _why = self._identity_closure(
                TypeRef(namespace, "predicate", declared), {}
            )
            for ref in closure:
                if ref.name == declared:
                    continue
                live = self.adapter.get_type(namespace, ref.name, kind="predicate")
    ...
    def _alias_holder(
        ...
        so **[Observed]** on `DegradedAdapter(page_cap=3)` over ten active rows a
