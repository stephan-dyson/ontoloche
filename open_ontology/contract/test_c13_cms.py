...
from ..types import Proposal, TypeEntry
from ._support import FIXTURE, SEVERITY_ORDERING, load_cms_sample, sample_facts
...
@pytest.fixture
def loaded(registry):
    return registry, load_cms_sample(registry)
...
def test_c13_01_the_samples_vocabulary_loads_as_eight_type_rows(loaded, adapter):
    ...
    assert adapter.find_consumers("default") == [], (
        "nothing in a CSV registers a consumer, and consumers() says so"
    )
    assert registry.consumers("facility").known == 0
...
def test_c13_02_usage_counts_match_the_pre_registered_ground_truth(loaded, record_property):
    ...
    for name, expected in PREREGISTERED.items():
        assert registry.usage(name).count == expected, f"{name} instance count"

    # 4 of the 6 full-file correction statuses appear in this slice -- T1, [Observed].
    assert registry.usage("deficiency_corrected_status").count == 4

    # The edges follow the grain of the two counts above.
    assert registry.usage("issued_during").count == PREREGISTERED["citation"]
    assert registry.usage("conducted_at").count == PREREGISTERED["survey"]
    ...
    assert registry.usage("scope_severity_code").count == computed
    print(
