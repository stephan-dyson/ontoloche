...
def test_c6_09_an_identity_written_only_as_an_alias_is_found_both_ways(
    adapter, make_registry
):
    ...
    **UC3:** an export's alias column is how a legacy word survives, and `import_types`
    ...
    """
    registry = make_registry(adapter, approval_policy="auto")
    seed(registry, "geo_scoped", kind="predicate", definition="a capability")
    seed(registry, "bike_rack", predicates=["geo_scoped"])
    seed(registry, "street_segment", predicates=["geo_scoped"])
    ...
    survivor = registry.list_types(namespace="default", predicate="geo_scoped")
    absorbed = registry.list_types(namespace="default", predicate="borough_scoped")
