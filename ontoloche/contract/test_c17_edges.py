...
def blocks(registry, **kw):
    return edge_family(registry, "blocks", inverse_label="blocked_by", **kw)


# --------------------------------------------------------------- the protocol itself


def test_c17_01_eighteen_primitives_and_no_edge_store_refuses_rather_than_raising(
    ...
    primitives = {n for n in vars(StorageAdapter) if not n.startswith("_")}
    ...
    if caps.stores_edges:
        blind = make_registry(DegradedAdapter(adapter, stores_edges=False, why=NO_EDGES))
    else:
        blind = make_registry(adapter)
    ...
    # ordinary statements.
    outcomes = [
        blind.neighbors(task(1), None, 1, namespace="default"),
        blind.add_edge("blocks", task(1), task(2), "user:sd"),
        blind.retract_edge("e1", "no", retracted_by="user:sd"),
        blind.edge_provenance("e1"),
    ]
    for out in outcomes:
    ...
    if not caps.stores_edges:
        with pytest.raises(NotSupported):
            adapter.find_edges(EdgeQuery())


@pytest.mark.requires_capability(
    "stores_edges", "stores_attributes", "stores_edge_attributes"
)
def test_c17_02_an_edge_record_round_trips_and_a_lost_payload_key_is_absent_not_wrong(
...
def test_c17_03_put_edge_stores_the_status_and_the_tombstone_and_judges_no_transition(
    adapter, clock
):
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_04_duplicate_edges_are_permitted_and_that_is_a_decision(adapter, make_registry):
    ...
    **The cost is asserted too**: `known` counts edges and not distinct neighbours, and
    ...
    registry = make_registry(adapter)
    blocks(registry)
    first = registry.add_edge("blocks", task(1), task(2), "user:sd", created_by="user")
    ...
    report = registry.neighbors(task(1), ["blocks"], 1, namespace="default")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_05_keyset_pagination_actually_pages(adapter, make_registry):
    ...
    blocks(registry)
    for i in range(2, 9):
        registry.add_edge("blocks", task(1), task(i), "user:sd")

    key = ("tenshen", "entity", "task", "1")
    seen: list[str] = []
    pages, cursor = 0, None
    while True:
        page = adapter.find_edges(EdgeQuery(incident_to=(key,), limit=3, after=cursor))
        pages += 1
    ...
    unpaged = adapter.find_edges(EdgeQuery(incident_to=(key,)))
...
def test_c17_06_the_family_filter_degrades_the_way_edges_says_and_not_the_way_types_does(
    adapter, make_registry
):
    ...
    flag in a second `DegradedAdapter` that declines it again proves nothing.
    """
    registry = make_registry(adapter)
    blocks(registry)
    edge_family(registry, "mentions")
    registry.add_edge("blocks", task(1), task(2), "user:sd")
    registry.add_edge("mentions", task(1), task(3), "user:sd")

    key = ("tenshen", "entity", "task", "1")
    indexed = adapter.find_edges(EdgeQuery(incident_to=(key,), families=("blocks",)))
    assert [r.family for r in indexed.records] == ["blocks"]
    assert indexed.complete is True

    unindexed = DegradedAdapter(
        adapter,
        indexes_edges_by_family=False,
        why={"indexes_edges_by_family": "work_links.relationship is free text with no index"},
    )
    page = unindexed.find_edges(EdgeQuery(incident_to=(key,), families=("blocks",)))
    ...
    above = make_registry(unindexed)
    narrowed = above.neighbors(task(1), ["blocks"], 1, namespace="default")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_07_a_type_level_endpoint_is_a_value_to_match_not_a_wildcard(adapter, make_registry):
    ...
    """
    registry = make_registry(adapter)
    edge_family(registry, "relates_to", level="type", src_kinds=("entity",), dst_kinds=("entity",))
    blocks(registry)
    registry.add_edge("relates_to", TASK, PERSON, "user:sd")
    registry.add_edge("blocks", task(1), task(2), "user:sd")

    type_page = adapter.find_edges(
        EdgeQuery(incident_to=(("tenshen", "entity", "task", None),))
    )
    assert [r.family for r in type_page.records] == ["relates_to"]

    instance_page = adapter.find_edges(
        EdgeQuery(incident_to=(("tenshen", "entity", "task", "1"),))
    )
    assert [r.family for r in instance_page.records] == ["blocks"]


# ------------------------------------------------------- 2.4.1, the endpoint rules


@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_08_the_level_check_runs_before_the_kind_check_and_the_detail_says_which(
    registry,
):
    ...
    """
    blocks(registry)
    refusal = registry.add_edge("blocks", TASK, task(2), "user:sd")
    ...
    edge_family(registry, "concerns", level="type", src_kinds=("entity",), dst_kinds=("entity",))
    kind_refusal = registry.add_edge(
...
def test_c17_10_a_symmetric_family_has_no_inverse_label_and_approve_is_where_it_is_checked(
    registry,
):
    ...
    rec = registry.adapter.get_proposal(good.id)
    registry.adapter.put_proposal(
        type(rec)(**{**rec.__dict__, "attributes": {**rec.attributes, "inverse_label": "x"}})
    )
    assert registry.approve(good.id, "user:sd").reason == "attributes_schema_violation"


@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_11_a_dangling_endpoint_is_a_fact_not_an_error(registry):
    ...
    **`endpoint_kind_mismatch` can only fire when the endpoint's type IS registered** --
    ...
    """
    blocks(registry)
    edge = registry.add_edge("blocks", task(1), task(2), "user:sd")
    ...
    seed(registry, "task", namespace="tenshen", definition="a unit of work")
    clean = registry.add_edge("blocks", task(5), task(6), "user:sd")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_12_the_depth_cap_is_two_and_three_raises(registry):
    ...
    """
    blocks(registry)
    one = registry.neighbors(task(1), ["blocks"], 1, namespace="default")
    two = registry.neighbors(task(1), ["blocks"], 2, namespace="default")
    assert (one.depth_requested, two.depth_requested) == (1, 2)
    for bad in (0, 3, 4):
        with pytest.raises(ValueError, match="depth must be"):
            registry.neighbors(task(1), ["blocks"], bad, namespace="default")



@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_13_edge_families_none_spans_every_namespace_and_an_unregistered_family_is_kept(
    adapter, registry
):
    ...
    **The value.** There is deliberately no foreign key from an edge to its family
    ...
    """
    blocks(registry)
    edge_family(registry, "owns", namespace="dpr")
    registry.add_edge("blocks", task(1), task(2), "user:sd")
    registry.add_edge("owns", task(1), task(3), "user:sd", namespace="dpr")

    scoped = registry.neighbors(task(1), None, 1, namespace="default")
    ...
    report = registry.neighbors(task(1), None, 1, namespace="default")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_14_a_typo_and_a_wrong_namespace_both_refuse_the_whole_call(registry):
    ...
    typo = registry.neighbors(task(1), ["blocs"], 1, namespace="default")
    assert isinstance(typo, Refusal) and typo.reason == "edge_family_unknown"
    assert typo.detail["families"] == ["blocs"]

    elsewhere = registry.neighbors(task(1), ["owns"], 1, namespace="default")
    assert isinstance(elsewhere, Refusal) and elsewhere.reason == "edge_family_unknown"

    mixed = registry.neighbors(task(1), ["blocks", "blocs"], 1, namespace="default")
...
def test_c17_15_a_retired_family_is_searched_and_the_caller_is_told(registry):
    ...
    blocks(registry)
    registry.add_edge("blocks", task(1), task(2), "user:sd")
    retired = registry.retire("blocks", "the classifier replaced it", retired_by="user:sd")
...
def test_c17_16_direction_filters_directed_families_only(registry):
    ...
    seed(registry, "borough", kind="value_set", namespace="dpr", definition="the five")
    seed(registry, "borough", kind="value_set", namespace="dot", definition="the five")
    registry.add_edge(EQUIVALENT_TO, dpr, dot, "user:sd")
    ...
    blocks(registry)
    registry.add_edge("blocks", task(1), task(2), "user:sd")
    out = registry.neighbors(task(1), ["blocks"], 1, namespace="default", direction="out")
    into = registry.neighbors(task(1), ["blocks"], 1, namespace="default", direction="in")
    assert out.known == 1 and into.known == 0

    mixed = registry.neighbors(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_17_a_dead_end_is_complete_and_truncation_is_a_separate_signal(registry):
    ...
    *arrived* on, and `depth_reached` was set whenever the scan returned any record. So
    ...
    barren = registry.neighbors(task(404), ["blocks"], 2, namespace="default")
    assert barren.known == 0 and barren.depth_reached == 0 and barren.complete is True


@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_18_the_assembly_bound_counts_distinct_edges_and_is_on_by_default(
    adapter, make_registry
):
    ...
    **B8, the opt-in.** 4.2 had measured the hazard, specified a mitigation and left it
    ...
    """
    registry = make_registry(adapter, max_edges=20)
    blocks(registry)
    hub = task("hub")
    leaves = [task(i) for i in range(10)]
    for leaf in leaves:
        registry.add_edge("blocks", hub, leaf, "user:sd")
    for a, b in zip(leaves, leaves[1:]):
        registry.add_edge("blocks", a, b, "user:sd")

    report = registry.neighbors(hub, ["blocks"], 2, namespace="default")
    ...
    bounded = make_registry(adapter, max_edges=5)
    tight = bounded.neighbors(hub, ["blocks"], 2, namespace="default")
    ...
    unbounded = make_registry(adapter, max_edges=None)
    assert unbounded.max_edges is None


@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_19_the_registry_exhausts_the_adapters_pages_and_a_stale_cursor_does_not_hang(
    adapter, make_registry
):
    ...
    **And a backend whose cursor never advances must not hang the walk.** `C0-10` asked
    ...
    blocks(registry)
    for i in range(300):
        registry.add_edge("blocks", task("hub"), task(i), "user:sd")

    paged = make_registry(DegradedAdapter(adapter, edge_page_cap=64))
    report = paged.neighbors(task("hub"), ["blocks"], 1, namespace="default")
    assert report.known == 300, "assembled from 64-row pages"
    assert report.complete is True

    stale = make_registry(
        DegradedAdapter(adapter, edge_page_cap=64, stale_edge_cursor=True)
    )
    stuck = stale.neighbors(task("hub"), ["blocks"], 1, namespace="default")
    assert stuck.complete is False
    assert "already returned" in stuck.why_incomplete


@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_20_retraction_is_an_event_never_a_delete(adapter, registry, make_registry):
    ...
    with pytest.raises(ValueError, match="non-empty reason"):
        registry.retract_edge(edge.edge_id, "   ", retracted_by="user:sd")

    missing = registry.retract_edge("no-such-edge", "gone", retracted_by="user:sd")
    ...
    second = trailless.add_edge("blocks", task(7), task(8), "user:sd")
    warned = trailless.retract_edge(second.edge_id, "still wrong", retracted_by="user:sd")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_21_every_edge_write_stamps_durability_itself_and_reads_stamp_nothing(
    adapter, make_registry
):
    ...
    """
    borrowed = DegradedAdapter(
        adapter,
        edge_transaction_scope="savepoint",
        why={"edge_transaction_scope": "the connection belongs to the host"},
    )
    registry = make_registry(borrowed)
    blocks(registry)

    added = registry.add_edge("blocks", task(1), task(2), "user:sd")
    ...
    owned = make_registry(adapter)
    durable = owned.add_edge("blocks", task(3), task(4), "user:sd")
    assert not any(w.startswith("not_durable_until_host_commits:") for w in durable.warnings)

    retracted = registry.retract_edge(durable.edge_id, "changed", retracted_by="user:sd")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_22_complete_is_readable_only_next_to_families_searched(registry):
    ...
    **The caveat is not small.** "Complete" is over `families_searched` and over the edge
    ...
    narrow = registry.neighbors(task(1), ["blocks"], 1, namespace="default")
    ...
    wide = registry.neighbors(task(1), ["blocks", "mentions"], 1, namespace="default")
    ...
    from ..edges import NeighborReport
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_23_self_loops_and_the_triangle_are_reachable_and_specified(registry):
    ...
    **`at_depth` is a property of the edge's DISCOVERY, not of a newly-reached node**:
    ...
    """
    blocks(registry)
    registry.add_edge("blocks", task("self"), task("self"), "user:sd")
    loop = registry.neighbors(task("self"), ["blocks"], 1, namespace="default")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_24_can_a_broken_edge_backend_pass(adapter, make_registry):
    ...
    dropper = make_registry(DegradedAdapter(adapter, drops_edge_limit=True))
    report = dropper.neighbors(task("hub"), ["blocks"], 1, namespace="default")
    assert report.known == 40, "a backend that ignores `limit` must not lose edges"
    assert report.complete is True

    stale = make_registry(DegradedAdapter(adapter, edge_page_cap=8, stale_edge_cursor=True))
    stuck = stale.neighbors(task("hub"), ["blocks"], 1, namespace="default")
    ...
    small = make_registry(DegradedAdapter(adapter, edge_page_cap=3), max_edges=45)
    whole = small.neighbors(task("hub"), ["blocks"], 2, namespace="default")
    assert whole.known == 40 and whole.complete is True


def test_c17_25_two_transaction_scopes_on_one_connection_is_non_conformant(adapter):
...
def test_c17_26_an_edges_history_is_append_only_and_says_when_it_could_not_be_read(
    ...
    registry.retract_edge(edge.edge_id, "changed", retracted_by="user:sd")
    provenance = registry.edge_provenance(edge.edge_id)
    ...
    blind = trailless.edge_provenance(edge.edge_id)
...
def test_c17_27_equivalent_to_is_seeded_with_the_exact_shape_the_spec_prints(registry):
    ...
    **`kind="edge"` IS a legal endpoint here and `predicate` is not**, and that
    ...
    it is the kill row.
    """
    entry = registry.adapter.get_type("default", EQUIVALENT_TO, kind="edge")
    ...
    surfaced = registry.provenance(EQUIVALENT_TO)
...
def test_c17_28_consumers_extends_to_edge_families_with_no_new_mechanism(registry):
    ...
    edge_family(registry, "waiting_on")
    bare = registry.consumers("waiting_on")
    ...
    seed(registry, "deadline_traversable", kind="predicate",
    ...
    family = registry.adapter.get_type("default", "blocks", kind="edge")
    claimed = type(family)(
        **{**family.__dict__, "predicates": ("deadline_traversable",)}
    )
    registry.adapter.put_type(claimed)
    ...
    report = registry.consumers("waiting_on")
    ...
    on_blocks = registry.consumers("blocks")
    assert [c.id for c in on_blocks.gates_on] == ["deadline_cluster_service.build"]


def test_c17_29_a_family_that_declared_nothing_is_a_legal_type_and_an_unusable_family(
    registry,
):
    ...
    """
    entry = seed(registry, "related_to", kind="edge", definition="an unspecified relationship")
    ...
    refusal = registry.add_edge("related_to", task(1), task(2), "user:sd")
    assert isinstance(refusal, Refusal)
    assert refusal.reason == "attributes_schema_violation"
...
# --------------------------------------------------- 4c: the amend path, ruling R37
#
# `EDGES.md` §5.2 narrated `edge_amended` with a worked example and v0 had no amend call
# at all -- deviation **D-4b-13**, question **Q32**. Ruling **R37** decides it *with*
# R34, because *"an edge whose payload is validated is an edge somebody will want to
# correct."* The path is taken; the design test that had to pass first is `C17-42`.


@pytest.mark.requires_capability(
    "stores_edges", "stores_attributes", "stores_edge_events", "stores_events"
)
def test_c17_41_an_amendment_is_a_new_event_carrying_the_old_and_new_values(registry):
    ...
    *"Changing an edge's `confidence` after a re-classification is a new `edge_amended`
    ...
    history = registry.edge_provenance(edge.edge_id)
...
@pytest.mark.requires_attribute_store
def test_c17_42_an_amendment_is_not_a_second_write_path(registry):
    ...
    still = registry.edge_provenance(edge.edge_id)
    ...
    registry.retract_edge(edge.edge_id, "written in error", retracted_by="user:sd")
    after = registry.amend_edge(
    ...
    with pytest.raises(ValueError):
        registry.amend_edge(edge.edge_id, "", amended_by="user:sd", confidence=1.0)
    with pytest.raises(ValueError):
        registry.amend_edge(edge.edge_id, "nothing to change", amended_by="user:sd")


@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_43_an_amendment_that_cannot_be_recorded_is_refused_and_a_retraction_is_not(
    adapter, make_registry
):
    ...
    **No new `Refusal.reason` is minted.** `cannot_record_override` is the fourth caller
    ...
    registry = make_registry(degraded)
    seed(registry, "task")
    blocks(registry)
    edge = registry.add_edge(
    ...
    retracted = registry.retract_edge(
...
# `stores_events` added by row 4d round 3: the cycle this test deliberately builds now
# needs `force=True` on its second leg (`retire(successor=)` refuses a RETIRED successor,
# `C9-25`), and a forced retirement needs somewhere to record the override. Scaffolding,
# not subject -- PACKAGE.md 8b.5.
@pytest.mark.requires_capability(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_47_a_second_retraction_is_refused_and_the_first_decision_survives(registry):
    ...
    **It is refused rather than made idempotent, and that is the ruling.** Idempotency
    ...
    standing = registry.edge_provenance(edge.edge_id)
    ...
    unknown = registry.retract_edge("no_such_edge", "x", retracted_by="user:sd")
...
# ------------------------------------------- 4c round 1: what the reviewers reproduced


@pytest.mark.requires_capability(
    "stores_edges", "stores_attributes", "stores_events"
)
def test_c17_48_a_written_reference_is_never_reported_as_a_followed_one(registry):
    ...
    """
    seed(registry, "hub")
    seed(registry, "owner", definition="the person accountable")
    seed(registry, "assignee", definition="the person accountable")
    blocks(registry)
    ...
    registry.add_edge("blocks", InstanceRef(hub, "9"), InstanceRef(owner, "7"), "user:sd")
    registry.add_edge(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes")
def test_c17_49_an_unregistered_family_edge_still_has_to_be_incident(
    adapter, make_registry
):
    ...
    *"caught incidentally is a weaker claim than pinned"* — one branch along**, in the id
    ...
    class _IgnoresFrontier:
        ...
        def find_edges(self, query):
            return self._inner.find_edges(replace(query, incident_to=None))

    registry = make_registry(adapter)
    seed(registry, "hub")
    blocks(registry)
    hub = TypeRef("default", "entity", "hub")
    registry.add_edge("blocks", InstanceRef(hub, "1"), InstanceRef(hub, "2"), "user:sd")
    ...
    adapter.put_edge(far, expect_absent=True)

    wide = make_registry(_IgnoresFrontier(adapter))
    report = wide.neighbors(InstanceRef(hub, "1"), None, 1, namespace="default")
...
@pytest.mark.requires_attribute_store
def test_c17_50_an_unrelated_amendment_does_not_erase_the_payloads_warnings(registry):
