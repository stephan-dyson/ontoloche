...
from ..actions import action_attributes
...
def test_c12_28_each_truncated_scan_in_this_method_names_itself(adapter, make_registry):
...
def test_c12_29_a_truncated_action_declaration_scan_says_so(adapter, make_registry):
    ...
    **And it is change A's consumer table one line short.** `8d717c9` enumerated five
    ...
    for i in range(6):
        seed(registry, f"verb_{i}", kind="action", definition="a verb",
             attributes=action_attributes(reversibility="reversible",
                                          approval_mode="auto"))

    capped = make_registry(DegradedAdapter(adapter, page_cap=3), approval_policy="auto")
    rows = capped.import_types(
