...
@pytest.mark.requires_capability("stores_events")
def test_c9_22_a_successor_that_does_not_exist_yet_is_refused(registry):
...
def test_c9_23_reinstate_asks_the_collision_question_its_sibling_asks(
    adapter, make_registry
):
    ...
    seed(registry, "aaa_note", predicates=["searchable", "taggable"])
    ...
    assert isinstance(
        registry.retire("searchable", "parked", retired_by="user:sd", force=True),
        TypeEntry,
    )
    ...
    # Nothing was written: a refusal is a refusal, not a warning on a completed act.
    assert registry.list_types(namespace="default").types
    assert [
...
@pytest.mark.requires_capability("stores_events")
def test_c9_25_a_retired_successor_leaves_the_old_word_resolving_to_nothing(registry):
    ...
    **Overridable**, exactly as §5.10's is — `force` is this call's acknowledgement —
    ...
    assert isinstance(
        registry.retire("mb", "gone", retired_by="user:sd", force=True), TypeEntry
    )

    refusal = registry.retire("ma", "folded", retired_by="user:sd", successor="mb")
    assert isinstance(refusal, Refusal) and refusal.reason == "retired_operand"
    assert refusal.detail["overridable"] is True
    assert registry.adapter.get_type("default", "ma").status == "active", "nothing written"
...
# ------------------------------- row 6c, adversarial round 1: the kill row's TWELFTH trip
#
# Found by the kill-row lens, briefed with all eleven prior trip records, and **proved to
# be introduced by this row's own item 5 by bisect** against `664d3a5^` rather than
# asserted: zero rows answering to the transferred word before the commit, two after.


@pytest.mark.requires_capability("stores_aliases", "indexes_membership", "stores_events")
def test_c9_29_a_second_retirement_is_a_no_op_and_writes_no_alias_onto_a_second_successor(
    adapter, make_registry
):
    ...
    **Fixed the way the sibling call already answers the identical question**, and the
    ...
    for name in ("alpha", "beta", "gamma"):
        seed(registry, name, kind="predicate", definition="one and the same thing")
    for member in ("aaa_note", "bbb_memo"):
        seed(registry, member, predicates=["alpha", "beta", "gamma"])
    rows = registry.import_types(
    ...
    live = {t.name: tuple(t.aliases) for t in registry.list_types("predicate").types}
...
@pytest.mark.requires_capability("stores_aliases", "indexes_membership", "stores_events")
def test_c9_30_the_successors_own_name_is_not_transferred_into_its_own_aliases(
    adapter, make_registry
):
...
@pytest.mark.requires_capability("stores_aliases", "indexes_membership", "stores_events")
def test_c9_31_the_alias_transfer_is_its_own_event_and_not_a_retirement(
    adapter, make_registry
):
    ...
    survivor = registry.provenance("taggable", namespace="default")
...
def test_c9_37_reinstate_refuses_a_word_a_tombstone_still_answers_to(
    adapter, make_registry
):
    ...
    **[Observed]** before the fix, four ordinary calls, no `force` and no adapter write:
    ...
    seed(registry, "beta", kind="entity", definition="a quite different beta definition")
    gone = registry.retire("alpha", "superseded", retired_by="user:sd", successor="beta")
    if isinstance(gone, Refusal):
        pytest.skip(f"this backend cannot retire toward a successor ({gone.reason})")
    gone2 = registry.retire("beta", "the area was dropped", retired_by="user:sd")
    if isinstance(gone2, Refusal):
        pytest.skip(f"this backend cannot retire the successor ({gone2.reason})")

    held = adapter.get_type("default", "beta", kind="entity")
    ...
    back = registry.reinstate("alpha", "we need it back", reinstated_by="user:sd")
...
def test_c9_38_a_predecessor_does_not_block_the_row_it_gave_its_words_to(
    adapter, make_registry
):
    ...
    **handed its words to beta** by its own governance act, so alpha's claim is the stale
    ...
    back = registry.reinstate("beta", "we need beta back", reinstated_by="user:sd")
