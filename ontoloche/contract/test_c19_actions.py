...
from ..types import REFUSAL_REASONS, Evidence, Proposal, Refusal, TypeEntry
...
@NEEDS_ATTRIBUTES
def test_c19_74_a_co_tenants_surface_cannot_make_a_neighbours_projection_a_typo(
    adapter, make_registry
):
    ...
    *"somebody else's namespace"* is the ordinary condition rather than the exotic one --
    ...
    registry = make_registry(adapter)
    for i in range(4):
        action_family(registry, f"ingest_dataset_{i}", reachability=[], namespace="dpr")

    alone = registry.projection(
...
# ----------------------------------------------------- round 3's findings, pinned
#
# The kill-row lens, briefed with all TEN trip records and pointed at this row's own
# 6B-RUN.md 6.2 prediction. Round 2's kill-row lens died on a rate limit without
# reporting, so these questions were being asked of this code for the first time.


@NEEDS_ATTRIBUTES
def test_c19_71_the_kind_on_a_reference_is_a_claim_and_the_stored_row_is_the_fact(
    adapter, make_registry
):
    ...
    **A ref naming no registered row is deliberately NOT refused here.** §1's last
    ...
    if registry.caps.stores_invocations:
        assert registry.invocations().known == 0, "nothing was written by any of the three"
    ...
    seed(registry, "task", kind="entity")
    action_family(registry, "ordinary", inputs=[InputSpec("a", "type")])
...
@NEEDS_ATTRIBUTES
def test_c19_72_the_payload_schema_binds_at_both_invocation_doors(adapter, make_registry):
    ...
    **A gate a recorder overrules is worse than no gate**: it tells a host the action may
    ...
    action_family(registry, "close_ticket", payload_schema="ticket_inputs")

    gate = registry.preflight("close_ticket", {}, actor="user:sd")
...
@NEEDS_ATTRIBUTES
def test_c19_73_a_precondition_names_a_predicate_by_the_registrys_notion_of_one_word(
    adapter, make_registry
):
    ...
    **The direction was safe** — Rule U, `holds=None`, verdict refused — so this is not a
    ...
    seed(registry, "commentable_", kind="predicate")
...
# ------------------------------------------------- row 6c's amendments (R70-R73, R75)
#
# `C19-74` sits with the other §10 ids above, next to the rule it narrows. These are the
# rest, each in its own rule's neighbourhood by number rather than by position, because
# renumbering an existing id is what §14 says a build row must never do.


@NEEDS_ATTRIBUTES
@pytest.mark.requires_capability("stores_edges", "stores_events")
def test_c19_75_preflight_warns_when_a_declared_edge_family_has_been_retired(
    adapter, make_registry
):
    ...
    **A warning and never a refusal**: refusing would make an ordinary retirement break
    ...
    """
    registry = make_registry(adapter)
    edge_family(registry, "person_links", level="instance")
    action_family(
        registry,
        "link_people",
    ...
    before = registry.preflight("link_people", {}, actor="user:sd")
    ...
    after = registry.preflight("link_people", {}, actor="user:sd")
    ...
    both = registry.preflight("link_and_unlink", {}, actor="user:sd")
    assert both.warnings == ("edge_family_retired:person_links",)


@NEEDS_INVOCATIONS
@pytest.mark.requires_capability("stores_edges", "stores_events")
def test_c19_76_record_invocation_warns_on_a_retired_declared_edge_family(
    adapter, make_registry
):
    ...
    list off `Registry._retired_blast_radius`.
    ...
    **Judged over the DECLARATION OF RECORD**, not over today's family: rule 3-7's own
    ...
    judged = registry.preflight("link_people", {}, actor="user:sd")
    assert judged.warnings == ()

    clean = registry.record_invocation(
    ...
    registry.retire("person_links", reason="superseded", retired_by="user:sd", force=True)

    late = registry.record_invocation(
        "link_people", {}, actor="user:sd", outcome="applied",
...
@NEEDS_INVOCATIONS
def test_c19_79_review_invocation_is_a_fifth_call_and_never_a_write_call_parameter(
    adapter, make_registry
):
...
@NEEDS_INVOCATIONS
def test_c19_80_an_unknown_invocation_id_refuses_unknown_invocation_and_not_the_family(
    adapter, make_registry
):
    ...
    **Not `action_family_unknown`**, which the build row reused and recorded as a
    ...
    out = registry.review_invocation("no-such-invocation", reviewed_by="user:boss")
...
@NEEDS_INVOCATIONS
def test_c19_81_the_review_queue_drains_end_to_end(adapter, make_registry):
    ...
    action_family(registry, "infer_person_relationships", approval_mode="review")
    gate = registry.preflight("infer_person_relationships", {}, actor="ai:nightly")
    filed = [
        registry.record_invocation(
    ...
    assert registry.invocations(unreviewed=True).known == 3

    for i, one in enumerate(filed):
        reviewed = registry.review_invocation(one.invocation_id, reviewed_by="user:boss")
        assert not isinstance(reviewed, Refusal), reviewed
        assert reviewed.reviewed_at is not None
        assert registry.invocations(unreviewed=True).known == 2 - i
    ...
    again = registry.review_invocation(filed[0].invocation_id, reviewed_by="user:other")
    assert not isinstance(again, Refusal), again
    assert registry.invocations(unreviewed=True).known == 0


# ------------------------------------------- row 6c, adversarial round 1 (integrator)
#
# Every finding below was CONSTRUCTED and RUN against the shipped registry by a lens
# that had never read this file. Four of the five are in the READ half or in *what the
# code does when nobody passed the optional argument* -- which is the fourth consecutive
# row that sentence has been true of the integrator lens.


@NEEDS_INVOCATIONS
def test_c19_82_a_ref_the_flat_form_cannot_carry_is_refused_at_both_doors(
    adapter, make_registry
):
    ...
    **The refusal belongs at the WRITE door and not in the parser**: a parser that
    ...
    registry = make_registry(adapter)
    seed(registry, "person")
    action_family(registry, "touch", inputs=[InputSpec("who", "type")])
    ...
    action_family(registry, "touch_one", inputs=[InputSpec("who", "instance")])
    out = registry.record_invocation(
...
@NEEDS_INVOCATIONS
@pytest.mark.requires_capability("stores_edges", "stores_events")
def test_c19_83_an_input_determined_effect_is_judged_over_the_inputs_namespaces(
    adapter, make_registry
):
    ...
    **[Observed]** with the family retired in the namespace the inputs point at and
    ...
    """
    registry = make_registry(adapter)
    seed(registry, "person", namespace="tenant_a")
    edge_family(registry, "person_links", level="instance", namespace="tenant_a")
    edge_family(registry, "person_links", level="instance", namespace="default")
    action_family(
    ...
    assert registry.preflight("link_people", who, actor="user:sd").warnings == ()
    ...
    gate = registry.preflight("link_people", who, actor="user:sd")
    ...
    quiet = registry.preflight("link_people", elsewhere, actor="user:sd")
...
@NEEDS_INVOCATIONS
def test_c19_84_the_review_queue_reads_the_mode_of_record_not_todays_mode(
    adapter, make_registry
):
    ...
    **[Observed]** three invocations filed under `review` and never reviewed left the
    ...
    """
    registry = make_registry(adapter)
    action_family(registry, "merge_contacts", approval_mode="review")
    gate = registry.preflight("merge_contacts", {}, actor="ai:nightly")
    for _ in range(3):
        registry.record_invocation(
    ...
    action_family(registry, "merge_contacts", approval_mode="auto")
    after = registry.invocations(unreviewed=True)
...
@NEEDS_INVOCATIONS
def test_c19_85_review_invocation_honours_its_namespace_and_needs_a_reviewer(
    adapter, make_registry
):
    ...
    """
    registry = make_registry(adapter)
    action_family(registry, "reconcile", approval_mode="review", namespace="dpr")
    gate = registry.preflight("reconcile", {}, actor="user:sd", namespace="dpr")
    filed = registry.record_invocation(
    ...
    wrong = registry.review_invocation(
    ...
    for empty in ("", "   "):
        with pytest.raises(ValueError):
            registry.review_invocation(filed.invocation_id, reviewed_by=empty)
    ...
    right = registry.review_invocation(
...
@NEEDS_ATTRIBUTES
def test_c19_86_projection_names_its_scope_and_says_which_empty_it_is(
    adapter, make_registry
):
    ...
    **the call affirmatively answering *everything fits* about a scope holding
    ...
    """
    registry = make_registry(adapter)
    action_family(registry, "publish", reachability=["console"], namespace="dpr")
    action_family(registry, "close", reachability=["console"], namespace="oti_311")

    scoped = registry.projection("s", budget=10, order=("console",), namespace="dpr")
    assert scoped.namespace == "dpr", "the report echoes the scope it answered over"
    assert scoped.counts == {"console": 1}

    wide = registry.projection("s", budget=10, order=("console",))
    ...
    absent = registry.projection("s", budget=10, order=("console",), namespace="dpr_typo")
    ...
    action_family(registry, "ingest", reachability=[], namespace="pipeline")
    quiet = registry.projection(
    ...
    for budget, reserved in ((-5, 0), (10, -1), (3, 4)):
        with pytest.raises(ValueError):
            registry.projection("s", budget=budget, reserved=reserved, order=("console",))


@NEEDS_INVOCATIONS
@pytest.mark.requires_capability("stores_edges", "stores_events")
def test_c19_87_an_unrelated_input_does_not_drag_its_namespace_into_the_warning(
    adapter, make_registry
):
    ...
    **[Observed]** `person_links` ACTIVE in `default` where the edge lands, retired in
    ...
    registry = make_registry(adapter)
    seed(registry, "person")
    seed(registry, "cfg", namespace="tenant_a")
    edge_family(registry, "person_links", level="instance")
    edge_family(registry, "person_links", level="instance", namespace="tenant_a")
    action_family(
        registry, "link_people",
    ...
    gate = registry.preflight("link_people", inputs, actor="user:sd")
    ...
    still = registry.preflight("link_people", only_a, actor="user:sd")
...
def test_c19_88_the_scope_census_tells_an_all_retired_scope_from_a_typod_one(
    adapter, make_registry
):
    ...
    *which* empty this is earns nothing if it cannot tell them apart.
    """
    registry = make_registry(adapter)
    action_family(registry, "publish_doc", reachability=["console"], namespace="tenant_a")
    assert not isinstance(
        registry.retire(
...
@NEEDS_INVOCATIONS
def test_c19_89_the_review_queues_rule_u_fallback_is_stated_and_counted(
    adapter, make_registry
):
    ...
    **today**, which is the entire defect `C19-84` closed, fully live for that row. The
    ...
    """
    registry = make_registry(adapter)
    action_family(registry, "publish", approval_mode="review")
    filed = registry.record_invocation(
        "publish", {}, actor="user:sd", outcome="applied", approved_by="user:sd",
    )
    assert not isinstance(filed, Refusal), filed

    ordinary = registry.invocations(unreviewed=True)
    ...
    stripped = list(registry.adapter.find_invocations(namespace="default").records)
    assert stripped, "the fixture filed one"
    blind = make_registry(_NoPolicyOfRecord(adapter))
    fell_back = blind.invocations(unreviewed=True)
...
def test_c19_90_the_flat_form_refusal_says_which_of_its_two_failures_this_is():
    ...
    **[Observed]** ``"org:beacon:entity:person"`` -> ``ValueError`` naming four
    ...
    """
    from ..actions import flat_form_problem
