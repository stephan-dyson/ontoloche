...
from .doubles import DegradedAdapter, OrderedAdapter, by_name_order
...
def test_c10_20_a_merge_may_not_write_a_word_another_live_row_already_answers_to(
    adapter, make_registry
):
    ...
    **Non-overridable, and `retired_operand` may not buy past it**: that value is an
    ...
    seed(registry, "ccc_draft", predicates=["beta"])
    ...
    live = {
        t.name: tuple(t.aliases) for t in registry.list_types("predicate").types
    }
    ...
    plain = make_registry(adapter)
    for name in ("p_one", "q_two"):
        seed(plain, name, kind="predicate", definition="one and the same thing")
    for member in ("n_one", "n_two"):
        seed(plain, member, predicates=["p_one", "q_two"])
    ok = plain.merge_types(
...
@pytest.mark.requires_capability("indexes_membership")
def test_c10_22_a_truncated_collision_scan_is_reported_not_silent(adapter, make_registry):
    ...
    for name in ("gamma", "beta"):
        seed(registry, name, kind="predicate", definition="a capability")
    seed(registry, "aaa_note", predicates=["gamma", "beta"])
    ...
    capped = make_registry(DegradedAdapter(adapter, page_cap=2), approval_policy="auto")
    words, why = capped._alias_clash("default", "beta", "predicate", ("gamma",))
    if why is None:
        pytest.skip("this backend answered the scan in full, so there is no `why` to drop")

    out = capped.merge_types(
...
def test_c10_23_the_escape_is_evaluated_over_the_whole_holder_set(adapter, make_registry):
    ...
    **non-overridably**. A non-overridable identity guard whose answer is a function of
    ...
    for i, order in enumerate(
        ...
        seed(ordered, "gamma_", kind="entity", definition="the escape firer", namespace=ns)
        gone = ordered.retire(
            "gamma", "no longer used", retired_by="user:sd", namespace=ns, force=True
        )
        if isinstance(gone, Refusal):
            pytest.skip(f"this backend cannot retire the holder ({gone.reason})")
        live = adapter.get_type(ns, "delta", kind="predicate")
        assert live is not None
        adapter.put_type(TypeRecord(**{**live.__dict__, "aliases": ("zeta",)}))

        out = ordered.merge_types(
        ...
        holders = [
            t.name
            for t in ordered.list_types("predicate", namespace=ns).types
...
def test_c10_24_merge_states_a_skipped_identity_guard_too(adapter, make_registry):
...
def test_c10_25_the_escape_excuses_only_left_itself(adapter, make_registry):
    ...
    **non-overridably** in the same store.
    ...
    seed(registry, "alpha_", kind="entity", definition="a thing")
...
def test_c10_26_the_refusal_detail_does_not_depend_on_page_order(
    adapter, make_registry
):
    ...
    for i, order in enumerate((("delta", "epsilon"), ("epsilon", "delta"))):
        ...
        for name in ("delta", "epsilon"):
            live = adapter.get_type(ns, name, kind="predicate")
            if live is None:
                pytest.skip("this backend did not write the fixture rows")
            adapter.put_type(TypeRecord(**{**live.__dict__, "aliases": ("zeta",)}))
        holders, _why = ordered._alias_clash(ns, "beta", "predicate", ("zeta",))
...
def test_c10_27_a_skip_note_survives_the_refusal_that_follows_it(adapter, make_registry):
    ...
    **we could not check and the answer is no anyway** — Rule U at the surface rather than
    ...
    seed(degraded, "ent_a", kind="entity", definition="a thing", predicates=["meta_p"])
    seed(degraded, "ent_b", kind="entity", definition="a thing")
    seed(degraded, "ent_c", kind="entity", definition="a thing")
    out = degraded.register_consumer(
        Consumer(id="svc:meta", gate="meta_p", on_unknown="drop", owner="ops")
    )
    if isinstance(out, Refusal):
    ...
    live = adapter.get_type("default", "ent_c", kind="entity")
    if live is None:
        pytest.skip("this backend did not write the fixture row")
    adapter.put_type(TypeRecord(**{**live.__dict__, "aliases": ("ent_a",)}))

    refused = degraded.merge_types(
        "ent_a", "ent_b", "one and the same", merged_by="user:sd",
        acknowledge=("definitions_diverge", "no_consumer_evidence"),
    )
