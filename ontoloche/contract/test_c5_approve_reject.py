...
def test_c5_15_approve_holds_a_tombstones_own_name(adapter, make_registry):
    ...
    **It is this row's own pattern**, adopted by R93 as the register's reading: change 1
    ...
    seed(registry, "borough_", kind="predicate", definition="the borough of a park")
    gone = registry.retire("borough_", "consolidated", retired_by="user:sd", force=True)
    if isinstance(gone, Refusal):
        pytest.skip(f"this backend cannot retire the holder ({gone.reason})")
    ...
    assert not [
        t for t in registry.list_types("predicate").types if t.name == "borough"
    ], "nothing is written"
    assert isinstance(
        registry.reinstate("borough_", "we were wrong", reinstated_by="user:sd"),
        TypeEntry,
