# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit ontoloche/registry.py and run:
#
...
from ontoloche._clock import Clock, SystemClock
from ontoloche._resolve import DeterministicResolver, identity_key, same_word, Resolver
from ontoloche.aio.adapter import (
    InvocationRecord,
    AttrObservedRecord,
    AttrSchemaRecord,
    AsyncAttributeStore,
...
from ontoloche.attributes import (
    ADDITIONAL,
    MODES,
    AttributeCensus,
    AttributeSchema,
...
from ontoloche.actions import family_declaration_problem as action_declaration_problem
...
from ontoloche.registry import (
...
class AsyncRegistry:
        ...
        **The outcome is still decided inside ``namespace`` alone.** A hit in another
        ...
        if exact is not None and exact.status == "retired":
            ...
            if successor:
                ...
                if live is not None and live.status != "retired":
                    ...
                    stale, agreement = await self._identity_agreement(namespace, exact, live)
        ...
        if best_score is not None and best_score >= policy.existing_threshold and best_score >= min_confidence:
            ...
            if (
                ...
                stale, agreement = await self._identity_agreement(namespace, written, entry)
    ...
    async def retire(
        ...
        *,
        ...
        if not report.gates_on and not self.caps.indexes_membership and not force:
            ...
            # in round 3). **[Observed, `AsyncDegradedAdapter(indexes_membership=False)`]**
    ...
    async def reinstate(
        ...
        **Ruling R19: this covers edge FAMILIES and never edge instances.** An edge
        ...
        tomb_row, tomb_word = tomb
        if tomb_row is not None and await self._hands_words_to(
    ...
    def _declared_identities(key: str, raw) -> frozenset:
        ...
        **What is deliberately NOT decided here.** Every parsed field is part of the
        ...
        """
        shape = AsyncRegistry._PARSED_DECLARED_KEYS[key]
        out = []
        for item in raw or ():
            try:
                parsed = item if isinstance(item, shape) else shape.from_dict(item)
                out.append(AsyncRegistry._parsed_identity(parsed))
    ...
    async def _hands_words_to(self, namespace: str, holder, name: str, kind: str) -> bool:
        ...
        **neither** ever comes back, which the guard above records as measured rather than
        ...
        while current and current not in seen and hops < _IDENTITY_CHAIN_CAP:
            ...
            seen.add(current)
            hops += 1
            nxt = await self.adapter.get_type(namespace, current, kind=kind)
            if nxt is None:
                return False
    ...
    async def _input_refusal(self, family: ActionFamily, inputs: dict) -> Refusal | None:
        ...
        **both** layers and spent its own round 1 learning why; ACTIONS.md claimed to
        ...
        for name, ref in inputs.items():
            ...
            if shape in ("type", "instance"):
                ...
                stored = await self.adapter.find_types(
        ...
        **Rule U on what is still unfound.** A family the lookup cannot find in any
        ...
        for effect in effects:
            ...
            found = [
                rec
                for rec in [
                    await self.adapter.get_type(scope, effect.family, kind="edge")
                    for scope in where
                ]
                if rec is not None
            ]
    ...
    async def preflight(
        ...
        **And the gate is advisory by construction, which ACTIONS.md 4 states in the
        ...
        bad = await self._input_refusal(fam, inputs)
        if bad is not None:
            return bad
    ...
    async def record_invocation(
        ...
        **The TOCTOU gap is named rather than closed.** Between ``preflight`` returning
        ...
        warnings.extend(
            await self._retired_blast_radius(
                namespace,
                effects_of_record,
                inputs,
    ...
    async def projection(
        ...
        **The registry never decides which families reach a surface.** That is the
        ...
        census, census_complete, census_why = await self._scope_census(namespace)
        ...
        #
        # **[Observed, `AsyncDegradedAdapter(page_cap=1, page_cursor=True)`, three active
