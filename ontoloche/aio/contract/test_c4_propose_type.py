# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit ontoloche/contract/test_c4_propose_type.py and run:
#
...
from ontoloche.types import CREATED_BY, Citation, Evidence, Proposal, Refusal, ResolveContext, TypeEntry
...
async def test_c4_11_declaring_a_predicate_whose_identity_moved_is_warned(adapter, make_registry):
    ...
    **It is a warning and never a refusal**, and that is §5.4's own rule rather than a
    ...
    moved = await registry.propose_type(
        "memo",
    ...
    plain = await registry.propose_type(
        "card",
        "a card",
        [Evidence(kind="data", summary="a sample")],
        "user:sd",
    ...
    dangling = await registry.propose_type(
        "leaflet",
        "a leaflet",
        [Evidence(kind="data", summary="a sample")],
        "user:sd",
...
async def test_c4_13_two_legal_names_that_are_one_word_cannot_both_go_live(adapter, make_registry):
    ...
    await seed(registry, "bike_lane", kind="predicate", definition="a capability")
    again = (await registry.import_types(
    ...
    both_kinds = await registry.propose_type(
...
async def test_c4_15_a_word_a_tombstone_answers_to_is_not_free_at_propose_type(
    adapter, make_registry
):
    ...
    **Provenance by BISECT** (standing rule (a)): live at `ddf2f5e` and at `664d3a5^`,
    ...
    out = await registry.propose_type(
...
async def test_c4_16_a_tombstones_word_is_held_against_every_kind(adapter, make_registry):
    ...
    **[Observed, row 6d round 1]** five ordinary calls, no `force` on the mint, no
    ...
    same = await registry.propose_type(
    ...
    assert f"word_previously_retired:{tombstone.name}" in out.warnings, out.warnings
    assert not [
        t for t in (await registry.list_types("entity", include_retired=True)).types
        if t.name == "zzz_moved"
...
async def test_c16_07_two_live_rows_may_still_share_a_word_across_kinds(adapter, make_registry):
    ...
    if not isinstance(out, TypeEntry):
        ...
        approved = await registry.approve(out.id, "user:sd")
        assert isinstance(approved, TypeEntry), approved
    live = {
        (t.kind, t.name)
        for t in (await registry.list_types(include_retired=True)).types
        if t.name == "beta"
    }
