# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit ontoloche/contract/test_c17_edges.py and run:
#
...
import pytest
from ontoloche.aio.adapter import EdgeQuery, EdgeRecord
from ontoloche.edges import (
    EQUIVALENT_TO,
...
from ontoloche.errors import NotSupported
from ontoloche.aio.registry import _IDENTITY_CHAIN_CAP
from ontoloche.types import Evidence, Refusal, TypeEntry
from ontoloche.aio.contract._support import edge_family, seed
...
async def test_c17_01_eighteen_primitives_and_no_edge_store_refuses_rather_than_raising(
    ...
    """
    from ontoloche.aio.adapter import AsyncStorageAdapter
...
async def test_c17_22_complete_is_readable_only_next_to_families_searched(registry):
    ...
    **The caveat is not small.** "Complete" is over `families_searched` and over the edge
    ...
    from ontoloche.edges import NeighborReport
...
async def _payload_family(registry, *, schema_name="blocks_payload", namespace="default"):
    await seed(registry, "task", namespace=namespace)
    return await edge_family(
        registry,
        "blocks",
...
async def test_c17_35_an_enforced_payload_schema_refuses_the_edge_and_names_which_schema(
    registry,
):
    ...
    **No new `Refusal.reason` is minted and that is the point.** This is
    ...
    refusal = await registry.add_edge(
    ...
    missing = await registry.add_edge(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_edge_attributes")
@pytest.mark.requires_attribute_store
async def test_c17_36_warn_mode_writes_and_the_version_in_force_is_recorded_on_the_edge(
    registry,
):
    ...
    stored = await registry.neighbors(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_edge_attributes")
async def test_c17_37_a_payload_schema_nobody_registered_writes_the_edge_and_says_so(registry):
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_edge_attributes")
@pytest.mark.requires_attribute_store
async def test_c17_38_a_payload_schema_does_not_make_its_own_family_unregisterable(registry):
    ...
    **[Observed, row 4c, before the fix]** a payload schema `{"role": str}` with
    ...
    refused = await registry.add_edge(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_edge_attributes")
@pytest.mark.requires_attribute_store
async def test_c17_39_the_enforcement_floor_and_the_per_namespace_schema_both_apply(registry):
    ...
    * a family's named schema replaces the FIELDS and may not weaken the STRICTNESS. A
    ...
    await edge_family(registry, "relates_to")
    refusal = await registry.add_edge(
    ...
    await edge_family(registry, "blocks", inverse_label="blocked_by", payload_schema="soft")
    await registry.register_attribute_schema(
        AttributeSchema(
            namespace="default",
            kind="edge_payload",
    ...
    still_refused = await registry.add_edge(
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_edge_attributes")
@pytest.mark.requires_attribute_store
async def test_c17_40_the_census_enumerates_edge_payload_keys_and_stays_tri_state(registry):
    ...
    census = await registry.attribute_census(kind="edge_payload")
...
async def test_c17_44_the_chain_is_followed_at_every_hop_not_only_at_the_origin(registry):
    ...
    """
    await seed(registry, "task")
    await seed(registry, "person_old", definition="a person who works here")
    await seed(registry, "person_new", definition="a person who works here")
    await blocks(registry)
    await edge_family(registry, "stakeholder")
    ...
    from_task = await registry.neighbors(
...
async def test_c17_45_a_broken_or_looping_chain_stops_at_depth_and_says_so(
    ...
    registry = await make_registry(adapter)
    await seed(registry, "task")
    await seed(registry, "alpha", definition="a word")
    await seed(registry, "beta", definition="a word")
    await blocks(registry)
    ...
    await seed(long_registry, "task")
    await blocks(long_registry)
    names = [f"n{i:02d}" for i in range(_CHAIN_BEYOND_CAP)]
    for name in names:
        await seed(long_registry, name, definition="a word in a long chain")
    await long_registry.add_edge(
        "blocks",
    ...
    for here, nxt in zip(names, names[1:]):
        assert not isinstance(
            await long_registry.retire(here, "chain", retired_by="user:sd", successor=nxt),
            Refusal,
        )
    capped = await long_registry.neighbors(
    ...
    blind = await make_registry(AsyncDegradedAdapter(adapter, page_cap=0))
    partial = await blind.neighbors(
        InstanceRef(beta, "1"), ["blocks"], 1, namespace="default"
    )
...
async def test_c17_46_retire_with_a_successor_is_followed_exactly_as_a_merge_is(registry):
    ...
    """
    await seed(registry, "task")
    await seed(registry, "agency_old", definition="a city agency")
    await seed(registry, "agency_new", definition="a city agency")
    await blocks(registry)
    ...
    report = await registry.neighbors(InstanceRef(new, "1"), ["blocks"], 1, namespace="default")
    assert report.known == 1, (
    ...
    await seed(registry, "obsolete", definition="a word nobody replaced")
    await registry.add_edge(
        "blocks",
    ...
    assert not isinstance(
        await registry.retire("obsolete", "nobody uses it", retired_by="user:sd"), Refusal
    )
    plain = await registry.neighbors(
        InstanceRef(TypeRef("default", "entity", "obsolete"), "1"),
        ["blocks"],
        1,
        namespace="default",
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_events")
async def test_c17_51_merging_an_edge_family_does_not_orphan_its_edges(registry):
    ...
    **followed** it.
    ...
    await blocks(registry)
    await edge_family(registry, "stakeholder", definition="who has a stake in this task")
    await edge_family(registry, "task_stakeholder", definition="who has a stake in this task")
    task_ref = TypeRef("default", "entity", "task")
...
@pytest.mark.requires_capability("stores_edges", "stores_attributes", "stores_events")
async def test_c17_52_a_retired_origin_type_says_so_and_an_int_id_is_refused(registry):
    ...
    **compare unequal**; `add_edge` accepted the int; and `neighbors` with the int
    ...
    live = await registry.neighbors(
        InstanceRef(person, "7"), ["blocks"], 1, namespace="default"
    )
    assert not any(w.startswith("origin_type_retired") for w in live.warnings)

    assert not isinstance(
        await registry.retire("person", "we use `human` now", retired_by="user:sd", force=True),
        Refusal,
    )
    after = await registry.neighbors(
        InstanceRef(person, "7"), ["blocks"], 1, namespace="default"
    )
...
@pytest.mark.requires_attribute_store
async def test_c17_53_an_absorbed_family_name_is_not_a_hole_in_the_schema_floor(registry):
    ...
    await edge_family(registry, "blocks", definition="one task depends on another")
    merged = await registry.merge_types(
    ...
    through_survivor = await registry.add_edge(
    ...
    through_absorbed = await registry.add_edge(
