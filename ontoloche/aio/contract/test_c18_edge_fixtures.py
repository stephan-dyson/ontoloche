# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit ontoloche/contract/test_c18_edge_fixtures.py and run:
#
...
pre-registered numbers, driven through `ontoloche.AsyncRegistry` and a real backend on
...
from ontoloche.edges import EQUIVALENT_TO, InstanceRef, TypeRef
from ontoloche.types import Evidence, Refusal
from ontoloche.aio.contract._support import cms_edges, edge_family, load_cms_sample, nyc_edge_sample, seed
...
async def _write_cms_edges(registry) -> dict:
...
@pytest.mark.cms
async def test_c18_01_the_three_cms_families_and_every_pre_registered_edge_count(registry):
    ...
    performed.
    """
    await load_cms_sample(registry)
    written = await _write_cms_edges(registry)
    ...
    for family in ("issued_during", "conducted_at", "cites"):
        entry = await registry.adapter.get_type(CMS, family, kind="edge")
...
@pytest.mark.cms
async def test_c18_02_a_facilitys_surveys_and_citations_sum_to_the_ground_truth(registry):
    ...
    """
    await load_cms_sample(registry)
    await _write_cms_edges(registry)
    pairs = cms_edges()
...
@pytest.mark.cms
async def test_c18_03_the_deepest_chain_in_the_fixture_is_two_hops(registry):
...
@pytest.mark.cms
async def test_c18_04_a_value_set_is_not_an_instance_endpoint_and_the_class_stays_out(registry):
    ...
    **yes, they all fit**, and therefore the whole class is refused.
    ...
    permissive = await registry.propose_type(
    ...
    await edge_family(registry, "has_severity", definition="a citation's severity")
    write = await registry.add_edge(
        "has_severity",
    ...
    from ontoloche.aio.contract._support import read_sample
...
async def _seed_boroughs(registry) -> dict:
...
async def test_c18_05_three_borough_value_sets_a_chain_not_a_triangle_and_no_entailment(registry):
    ...
    """
    sample = await _seed_boroughs(registry)
    ...
    one = await registry.neighbors(dpr, [EQUIVALENT_TO], 1, namespace="default")
    assert one.known == 1 and one.complete is True
    assert [str(n) for n in one.nodes] == [str(oti)]

    two = await registry.neighbors(dpr, [EQUIVALENT_TO], 2, namespace="default")
...
async def test_c18_06_the_report_spans_namespaces_the_caller_never_named(registry):
    ...
    **And `complete=True` here means one thing only**: every `equivalent_to` edge in
    ...
    """
    await _seed_boroughs(registry)
    dpr = TypeRef("dpr", "value_set", "borough")
    oti = TypeRef("oti_311", "value_set", "borough")
    dot = TypeRef("dot", "value_set", "borough")
    await registry.add_edge(EQUIVALENT_TO, dpr, oti, "user:dot")
    await registry.add_edge(EQUIVALENT_TO, oti, dot, "user:dot")

    report = await registry.neighbors(dpr, [EQUIVALENT_TO], 2, namespace="default")
...
async def test_c18_07_an_equivalence_edge_does_not_weaken_the_merge_refusal(registry):
    ...
    dpr = TypeRef("dpr", "value_set", "borough")
    oti = TypeRef("oti_311", "value_set", "borough")
    written = await registry.add_edge(EQUIVALENT_TO, dpr, oti, "user:dot")
    ...
    acknowledged = await registry.merge_types(
        "borough", "borough", "the same five boroughs", merged_by="user:dot",
        namespace="dpr", into_namespace="oti_311",
    ...
    still_there = await registry.neighbors(dpr, [EQUIVALENT_TO], 1, namespace="default")
    assert still_there.known == 1, "and the edge is untouched by the refusal"

async def test_c18_08_a_deterministic_key_join_is_many_to_many_and_confidence_carries_it(
    registry,
):
    ...
    **many-to-many** -- one lot carries up to 16 census trees, and 7 of 25 tree
    ...
    """
    sample = nyc_edge_sample()
    await seed(registry, "street_tree", namespace="dpr", definition="a NYC census street tree")
    await seed(registry, "service_request", namespace="oti_311", definition="a 311 request")
    await edge_family(
...
async def test_c18_09_a_work_links_shaped_row_maps_onto_the_protocol(adapter, make_registry):
    ...
    **No beacon code, and nothing of beacon's is vendored.** The rows above are column
    ...
    registry = await make_registry(host)
    await edge_family(registry, "blocks", inverse_label="blocked_by")
    await edge_family(registry, "part_of", inverse_label="has_part")
    ...
    for row in WORK_LINKS_ROWS:
        ...
        if isinstance(mapped, Refusal):
            continue
        stored.append(await host.put_edge(mapped))
    ...
    if (await adapter.capabilities()).stores_edge_attributes:
...
async def test_c18_10_a_null_relationship_type_is_refused_explicitly_never_dropped(registry):
