# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit ontoloche/contract/test_c3_resolve_type.py and run:
#
...
import pytest
from ontoloche._resolve import identity_key
from ontoloche.actions import Effect, InputSpec, Precondition, action_attributes
from ontoloche.aio.contract.doubles import AsyncDegradedAdapter
from ontoloche.types import Refusal, Resolution, ResolveContext, TypeEntry
from ontoloche.aio.contract._support import seed, snapshot
...
async def test_c3_04_confidence_is_none_when_no_scorer_ran_and_none_is_not_zero(
    ...
    """
    registry = await make_registry(adapter, seed_equivalent_to=False)
    resolution = await registry.resolve_type("facility", ResolveContext(), tier="opus")
    assert resolution.outcome == "proposal"
...
async def test_c3_14_a_redirect_whose_identity_claim_went_stale_says_so(registry):
    ...
    **The confidence is untouched at 1.0 on purpose.** Refusing to answer, or answering
    ...
    near = await registry.resolve_type("commentables", ResolveContext(), tier="opus")
...
@pytest.mark.requires_capability("stores_events")
async def test_c3_16_the_successor_walk_never_raises_and_says_when_it_stopped(registry):
    ...
    **namespaces** in `why_incomplete`. `_identity_closure` and `list_types` both name
    ...
    assert (await registry.resolve_type(
    ...
    await seed(registry, "searchable", kind="value_set", definition="a different thing")
    resolution = await registry.resolve_type(
        "commentable", ResolveContext(), kind="entity", tier="opus"
    )
    ...
    for here, nxt in zip(names, names[1:]):
        assert isinstance(
            await registry.retire(here, "chain", retired_by="user:sd", successor=nxt, force=True),
            TypeEntry,
        )
    capped = await registry.resolve_type("pp00", ResolveContext(), tier="opus")
...
async def test_c3_19_every_alternative_label_names_a_row(adapter, make_registry):
    ...
    **In the call ruling R6 added to end confident NEGATIVES.** The general form is the
    ...
    registry = await make_registry(adapter, approval_policy="auto")
    await seed(registry, "bike__lane", namespace="dpr", definition="a cycle route")
    await seed(registry, "park", namespace="dpr", definition="a green space")
    await seed(registry, "roadway", namespace="dot", definition="a carriageway")
    ...
    for label, _score in out.alternatives:
        ...
        found = [
            t
            for t in (await registry.list_types(
...
async def _stale_predicate_store(registry):
...
@pytest.mark.requires_capability("indexes_membership")
async def test_c3_20_the_stale_confidence_is_derived_from_the_extents_not_chosen(registry):
    ...
    **This id exists to make a CONSTANT fail.** Row 6f pre-registered, before the
    ...
    """
    await _stale_predicate_store(registry)

    stale = await registry.resolve_type("commentable", ResolveContext(), tier="opus")
    left, _, _ = await registry._written_extent("default", "commentable", include_retired=True)
    right, _, _ = await registry._written_extent("default", "searchable", include_retired=True)
    ...
    for name in ("taggable", "labelable"):
        await seed(registry, name, kind="predicate", definition="a capability")
    await seed(registry, "post", predicates=["taggable", "labelable"])
    assert isinstance(
        await registry.retire("taggable", "superseded", retired_by="user:sd", successor="labelable"),
        TypeEntry,
    )
    for extra in ("page", "clip", "reel"):
        await seed(registry, extra, predicates=["labelable"])
    second = await registry.resolve_type("taggable", ResolveContext(), tier="opus")
    l2, _, _ = await registry._written_extent("default", "taggable", include_retired=True)
    r2, _, _ = await registry._written_extent("default", "labelable", include_retired=True)
...
async def test_c3_21_an_action_familys_identity_is_verified_by_its_declaration(registry):
    ...
    **A3 is NOT closed by this id.** Its write doors still let the collapse through on
    ...
    await seed(registry, "alpha_verb", kind="action", attributes=action_attributes(**common))
    await seed(registry, "beta_verb", kind="action", attributes=action_attributes(**common))
    assert isinstance(
        await registry.retire(
    ...
    agreeing = await registry.resolve_type("alpha_verb", ResolveContext(), tier="haiku")
    assert agreeing.confidence == 1.0, (
...
@pytest.mark.requires_capability("indexes_membership")
async def test_c3_22_a_stale_redirect_is_never_a_refusal(registry):
    ...
    **The id exists so that a later row cannot quietly turn the score into a refusal**
    ...
    """
    await _stale_predicate_store(registry)
    answer = await registry.resolve_type("commentable", ResolveContext(), tier="opus")
...
async def test_c3_23_min_confidence_governs_the_redirect_path(registry):
    ...
    *before* the `min_confidence` test at `1707`, so `min_confidence=2.0` still answered
    ...
    baseline = await registry.resolve_type("commentable", ResolveContext(), tier="opus")
    ...
    default = await registry.resolve_type("commentable", ResolveContext(), tier="opus")
    assert default.outcome == "existing", "min_confidence defaults to 0.0 -- no change"

    at_bar = await registry.resolve_type(
    ...
    above = await registry.resolve_type(
    ...
    await seed(registry, "clip", predicates=["labelable"])

    for spelling in ("taggable", "Taggable", "TAGGABLE"):
        under = await registry.resolve_type(
...
@pytest.mark.requires_capability("stores_attributes", "indexes_membership")
async def test_c3_24_a_reordered_declaration_is_the_SAME_declaration(registry):
    ...
    **That protection was UNPINNED until this id.** Round 1's mutation lens reverted the
    ...
    assert isinstance(
        await registry.retire("old_verb", "superseded", retired_by="user:sd", successor="new_verb"),
        TypeEntry,
    )
...
@pytest.mark.requires_capability("stores_attributes", "indexes_membership")
async def test_c3_25_an_UNDECLARED_action_family_is_unknowable_not_agreeing(registry):
    ...
    *"fall through to `(False, 1.0)`, so an undeclared family silently became FULLY
    ...
    """
    await seed(registry, "old_verb", kind="action", attributes={})
    await seed(
        registry,
        "new_verb",
        kind="action",
...
@pytest.mark.requires_capability("indexes_membership")
async def test_c3_26_an_unscorable_identity_does_not_clear_a_bar(registry, adapter, make_registry):
    ...
    unscored = await blind.resolve_type("commentable", ResolveContext(), tier="opus")
    ...
    barred = await blind.resolve_type(
...
async def test_c3_27_the_confidence_is_the_MIN_of_both_halves(adapter, make_registry):
    ...
    *unreachable by construction* -- tagging rule 5.3.2-9 `prose-only:`. **A round-2 lens
    ...
    for name in ("commentable", "searchable"):
        await seed(registry, name, kind="predicate", definition="a capability")
    for i in range(20):
        await seed(registry, f"shared{i:02d}", predicates=["commentable", "searchable"])
    merged = await registry.merge_types(
        "commentable", "searchable", "same capability", merged_by="user:sd",
        acknowledge=["definitions_diverge", "no_consumer_evidence"],
    )
    assert not isinstance(merged, Refusal), merged
    await seed(registry, "latecomer", predicates=["searchable"])
...
@pytest.mark.requires_capability("indexes_membership")
async def test_c3_28_an_unknowable_read_answers_None_not_zero(registry, adapter, make_registry):
    ...
    **This rule existed only in prose until round 2**, found by a supervisor spec-read
    ...
    scored = await registry.resolve_type("commentable", ResolveContext(), tier="opus")
    ...
    unknowable = await blind.resolve_type("commentable", ResolveContext(), tier="opus")
...
@pytest.mark.requires_capability("stores_attributes", "indexes_membership")
async def test_c3_29_a_key_declared_EMPTY_is_not_a_key_never_declared(registry):
    ...
    await seed(registry, "old_verb", kind="action", attributes=says_none)
    await seed(registry, "new_verb", kind="action", attributes=never_said)

    holds, agreement = await registry._identity_agreement(
        "default",
        await registry._require("default", "old_verb"),
        await registry._require("default", "new_verb"),
    )
    ...
    await seed(registry, "alpha_verb", kind="action", attributes=left_side)
    await seed(registry, "beta_verb", kind="action", attributes=right_side)
    _, shared_absent = await registry._identity_agreement(
        "default",
        await registry._require("default", "alpha_verb"),
        await registry._require("default", "beta_verb"),
    )
