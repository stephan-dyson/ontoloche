# ---------------------------------------------------------------------------------
# GENERATED FILE -- do not edit. Edit ontoloche/contract/test_c5_approve_reject.py and run:
#
...
from ontoloche.types import Evidence, Proposal, Refusal, Rejection, TypeEntry
...
async def test_c5_01_approve_records_who_and_when_and_activates(registry, clock):
    proposal = await registry.propose_type("facility", "a nursing home", [DATA_EVIDENCE], "user:pm")
...
async def test_c5_02_no_active_entry_anywhere_has_a_null_approver(registry, adapter, make_registry):
    ...
    await registry.import_types([{"name": "imported_thing", "status": "active"}])
...
@pytest.mark.requires_capability("stores_proposals")
async def test_c5_03_the_severity_case_verbatim(adapter, make_registry):
    ...
    refusal = await registry.approve(proposal.id, "ai:proposer", mode="auto")
...
@pytest.mark.requires_capability("stores_proposals")
async def test_c5_04_approving_a_decided_proposal_is_an_idempotent_refusal(registry):
    proposal = await registry.propose_type("facility", "a nursing home", [DATA_EVIDENCE], "user:pm")
    first = await registry.approve(proposal.id, "user:sd")
    assert isinstance(first, TypeEntry)

    second = await registry.approve(proposal.id, "user:other")
    ...
    third = await registry.reject(proposal.id, "user:other", "changed my mind")
    assert isinstance(third, Refusal) and third.reason == "already_decided"

@pytest.mark.requires_capability("stores_proposals")
async def test_c5_05_an_unknown_proposal_id_is_a_refusal(registry):
    refusal = await registry.approve("no-such-proposal", "user:sd")
    assert isinstance(refusal, Refusal) and refusal.reason == "unknown_proposal"

@pytest.mark.requires_capability("stores_proposals")
async def test_c5_06_approving_with_unverified_semantics_succeeds_and_keeps_the_warning(registry):
    proposal = await registry.propose_type(
        "deficiency_grade",
    ...
    listing = await registry.list_types(unverified_semantics=True)
...
async def test_c5_07_an_approvers_amendment_keeps_the_original_in_history(registry):
    proposal = await registry.propose_type(
    ...
    approved = [e for e in (await registry.provenance("facility")).history if e.event == "approved"]
    assert approved
...
@pytest.mark.requires_capability("stores_proposals")
async def test_c5_08_reject_requires_a_reason(registry):
    proposal = await registry.propose_type("widget", "a thing", [DATA_EVIDENCE], "user:pm")
    with pytest.raises(ValueError):
        await registry.reject(proposal.id, "user:sd", "")
    with pytest.raises(ValueError):
        await registry.reject(proposal.id, "user:sd", "   ")

@pytest.mark.requires_capability("stores_proposals")
async def test_c5_09_a_rejection_is_retained_and_findable(registry, adapter):
    """The record that stops a re-proposal in six months."""
    proposal = await registry.propose_type("widget", "a thing", [DATA_EVIDENCE], "user:pm")
    rejection = await registry.reject(proposal.id, "user:sd", "not a domain concept; use `component`")
    assert isinstance(rejection, Rejection)

    page = await adapter.find_proposals(ProposalQuery(namespace="default", status="rejected"))
...
@pytest.mark.requires_capability("stores_proposals")
async def test_c5_10_reject_records_the_successor(registry, adapter):
    await seed(registry, "component", definition="the type the proposer should have used")
    proposal = await registry.propose_type("widget", "a thing", [DATA_EVIDENCE], "user:pm")
    rejection = await registry.reject(
        proposal.id, "user:sd", "use the existing word", superseded_by="component"
    )
    assert rejection.superseded_by == "component"
    stored = await adapter.get_proposal(proposal.id)
    assert stored.superseded_by == "component"

@pytest.mark.requires_capability("stores_proposals", "stores_events")
async def test_c5_11_an_approval_is_atomic(registry, adapter, monkeypatch):
    ...
    monkeypatch.undo()
    assert await adapter.get_type("default", "facility") is None
    assert (await adapter.get_proposal(proposal.id)).status == "pending"
    assert (await adapter.get_proposal(proposal.id)).decided_by is None

async def test_c5_12_a_backend_with_no_proposal_table_refuses_to_decide(adapter, make_registry):
    ...
    """
    blind = await make_registry(AsyncDegradedAdapter(adapter, stores_proposals=False))

    # propose_type IS the decision -- an entry, immediately, auto-approved and legible.
    entry = await blind.propose_type(
    ...
    refused = await blind.approve("any-id-at-all", "user:sd")
    ...
    rejected = await blind.reject("any-id-at-all", "user:sd", "no")
...
async def test_c5_13_a_word_a_tombstone_answers_to_is_not_free_at_approve(
...
async def test_c5_14_approve_holds_a_tombstones_word_across_kinds(adapter, make_registry):
