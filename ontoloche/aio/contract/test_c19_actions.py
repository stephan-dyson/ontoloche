...
async def _verb(registry, name, *, mode, rev, definition="one and the same verb"):
...
async def test_c19_97_a_merge_refuses_two_contradictory_governance_declarations(
    adapter, make_registry
):
    ...
    **The definitions here are IDENTICAL, and that is the point.** The row's own first
    ...
    await _verb(registry, "old_verb", mode="auto", rev="reversible")
    await _verb(registry, "new_verb", mode="human", rev="irreversible")
...
async def test_c19_98_a_retirement_toward_a_contradictory_successor_is_refused(
    adapter, make_registry
):
...
async def test_c19_99_two_agreeing_families_still_merge(adapter, make_registry):
    ...
    if not registry.caps.stores_attributes:
        pytest.skip("this backend stores no attributes, so a family cannot be declared")
    await _verb(registry, "a_verb", mode="human", rev="irreversible")
    await _verb(registry, "b_verb", mode="human", rev="irreversible", definition="a different sentence entirely")
...
async def _verb_fx(registry, name, *, effects, mode="auto", rev="reversible", tier=None):
...
async def test_c19_100_effects_declared_in_a_different_order_are_the_same_declaration(
    adapter, make_registry
):
    ...
    if not registry.caps.stores_attributes:
        pytest.skip("this backend stores no attributes, so a family cannot be declared")
    await edge_family(registry, "person_links")

    fx = [_effect("add_edge", family="person_links"),
          _effect("propose_type", kind="entity")]
    await _verb_fx(registry, "pay_out", effects=fx)
    await _verb_fx(registry, "payout_", effects=list(reversed(fx)))
...
async def test_c19_101_a_why_on_a_protocol_op_is_not_part_of_effect_identity(
    adapter, make_registry
):
...
async def test_c19_102_host_state_why_IS_its_identity_and_still_refuses(
    adapter, make_registry
):
...
async def test_c19_103_an_extra_effect_and_a_differing_tier_still_refuse(
    adapter, make_registry
):
    ...
    await _verb_fx(registry, "pay_out", effects=base)
    await _verb_fx(registry, "payout_", effects=base + [_effect("propose_type", kind="entity")])
    extra = await registry.merge_types(
        "pay_out", "payout_", "one and the same", merged_by="user:sd",
        acknowledge=("no_consumer_evidence", "definitions_diverge"),
    )
    ...
    await _verb_fx(registry, "c_verb", effects=base, mode="auto", tier="ai:opus")
    await _verb_fx(registry, "d_verb", effects=base, mode="auto", tier="ai:haiku")
    tier = await registry.merge_types(
...
async def _verb_eight(registry, name, *, definition="one and the same verb", **overrides):
...
async def _eight_key_ground(registry):
    ...
    for pred in ("never_holds", "also_never_holds"):
        await seed(registry, pred, kind="predicate")
    for ent in ("guarded_thing", "other_thing"):
        await seed(registry, ent, kind="entity")

async def _merge_eight(registry, a, b):
    return await registry.merge_types(
...
async def test_c19_104_a_contradiction_on_preconditions_refuses_the_collapse(
    adapter, make_registry
):
    ...
    **That is the route by which entry `A3` fires this project's governance stop
    ...
    if not registry.caps.stores_attributes:
        pytest.skip("this backend stores no attributes, so a family cannot be declared")
    await _eight_key_ground(registry)
    await _verb_eight(registry, "old_verb")
    await _verb_eight(
        registry, "new_verb",
        preconditions=[
            Precondition(
    ...
    merged = await _merge_eight(registry, "old_verb", "new_verb")
...
async def test_c19_105_the_three_new_list_keys_are_order_insensitive_too(
    adapter, make_registry
):
    ...
    **Found by the round's second lens, which built the combined fixture and RAN it**
    ...
    for i, key in enumerate(("inputs", "preconditions", "reachability")):
        ...
        await _verb_eight(registry, left, **{key: list(forward)})
        await _verb_eight(registry, right, **{key: list(reversed(forward))})

        out = await _merge_eight(registry, left, right)
...
async def test_c19_106_an_absent_key_is_not_a_contradiction_on_the_four_R102_added(
    adapter, make_registry
):
    ...
    **What this id does NOT touch:** the four keys `_GOVERNANCE_KEYS` already compared.
    ...
    for i, declared in enumerate(([], ["mcp", "cli"])):
        ...
        await seed(registry, left, kind="action", definition="one and the same verb",
             attributes=absent)
        await seed(registry, right, kind="action", definition="one and the same verb",
             attributes=present)
...
async def test_c19_107_a_genuinely_different_inputs_refuses_and_a_respelt_one_does_not(
    adapter, make_registry
):
    ...
    **Both halves are asserted together deliberately.** An id that only pins the refusal
    ...
    await _verb_eight(registry, "pays_out")
    await _verb_eight(
        registry, "disburses",
        inputs=_eight_kwargs()["inputs"]
        + [InputSpec(name="third", ref="instance", kinds=("entity",))],
    )
    out = await _merge_eight(registry, "pays_out", "disburses")
    assert isinstance(out, Refusal), (
    ...
    await seed(registry, "draws_down", kind="action", definition="one and the same verb",
         attributes=full)
    await seed(registry, "withdraws", kind="action", definition="one and the same verb",
         attributes=respelt)

    respelt_out = await _merge_eight(registry, "draws_down", "withdraws")
...
async def test_c19_108_nested_membership_inside_a_declared_input_is_unordered_too(
    adapter, make_registry
):
    ...
    **The first attempt to reproduce it FAILED, and the reason is kept** because it is
    ...
    await seed(registry, "opens_gate", kind="action", definition="one and the same verb",
         attributes=forward)
    await seed(registry, "unlatches", kind="action", definition="one and the same verb",
         attributes=flipped)

    out = await _merge_eight(registry, "opens_gate", "unlatches")
    _skip_if_cannot_record(registry, out)
    assert not isinstance(out, Refusal), (
