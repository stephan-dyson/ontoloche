...
Point it at a Postgres with::
    ...
    OO_POSTGRES_DSN=postgresql://user:pw@localhost:5432/postgres pytest --pyargs ontoloche.aio.contract
"""
...
from ontoloche.aio.backends.sqlite import AsyncSQLiteAdapter
from ontoloche.aio.registry import AsyncRegistry
from ontoloche.edges import DEFAULT_MAX_EDGES
from ontoloche.policy import NamespacePolicy

from ontoloche.contract._coverage import Coverage
...
async def adapter_factory(backend, tmp_path_factory):
    ...
    if backend == "external":
        ...
        from ontoloche.aio.backends.sqlite_minimal import AsyncMinimalSQLiteAdapter
        ...
        from ontoloche.aio.backends.postgres import AsyncPostgresAdapter
...
async def adapter(adapter_factory, request, backend):
    ...
    if request.node.get_closest_marker("requires_attribute_store") is not None:
        from ontoloche.adapter import AttributeStore
    ...
    # **`iter_markers`, not `get_closest_marker` -- see the sync conftest's note.**
    # `get_closest_marker` returns exactly ONE mark, so stacking two
    # `@requires_capability` decorators silently discarded all but the innermost, and a
    # test declaring two needs was run in a configuration that cannot host it. This file
    # is NOT generated by `tools/unasync.py` (it transforms the registry, the adapter,
    # `_sql.py` and the test modules), so the fix is applied here by hand -- which is
    # exactly the *"a fix is only as good as its application"* shape the register names,
    # and the reason this comment says where the other copy is.
    caps = None
