...
log = logging.getLogger("ontoloche")
...
def _aware(value: datetime) -> datetime:
    """A naive datetime read as UTC. **Round 2's fix-auditor lens.**

    Fix 2 re-applies every filter above the store, which put a Python-side
    ``rec.created_at >= since`` where none existed before -- and primitive 21 accepts a
    naive ``since`` and answers, while the facade raised ``TypeError: can't compare
    offset-naive and offset-aware datetimes``. **A facade that crashes on a value its own
    primitive takes is a regression the fix introduced**, and the registry stores UTC
    throughout (`PACKAGE.md` 4.4), so reading a naive value as UTC is the same reading
    every adapter already makes of one.
    """
...
def _answers_to(rec, word: str) -> bool:
    """Does this row answer to ``word`` -- by NAME or by ALIAS, keyed rather than bytes?

    **Findings X1 and X2, row 6d round 1 (ruling R90).** `_search_namespaces` is the only
    guard in this package that reads more than one namespace, and it was built out of
    **none** of the identity machinery: it compared `rec.name == candidate`, so a word a
    live tombstone in another namespace still answered to **as an alias** was invisible --
    and invisible under a `complete=True` seal, which is Rule U's confident negative in
    the call ruling **R6** exists to prevent (*"scoping without lookup reintroduces
    mechanism 2"*).

    **[Observed]** live NYC Socrata headers: `dpr:boroname` retired holding
    `('borough', 'boro_nm')`, and `resolve_type('boro_nm')` from `oti_311` naming every
    namespace answered *"nothing in the vocabulary fits"* with `complete=True` and
    `why_incomplete=''`. The tombstone's own NAME was surfaced; the two words the same
    tombstone answers to were not.
    ...
    *unconsumed permission*. This is that rule at the one read that crosses a namespace.
    """
    return same_word(rec.name, word) or any(
        same_word(alias, word) for alias in (getattr(rec, "aliases", None) or ())
    )
...
class Registry:
    ...
    # ------------------------------------------------------------------ projections
    def _identity_names(self, rec: TypeRecord, cache: dict | None = None) -> tuple[str, ...]:
    ...
    def _gates_on(
        ...
        **fact**, not an unknown. Refusing instead would ban `import_types` from writing
        ...
        # can honestly claim.
        identity = (
            set(self._identity_names(rec))
            if kind == "predicate" and rec is not None
            else {name}
        )
        return {
            row.consumer_id
            for row in self.adapter.find_consumers(namespace)
        ...
        *,
        ...
        for rec in page.records:
            ...
            if wanted is not None and not (
                set(self._identity_names(rec, closure_cache)) & wanted
            ):
                continue
        ...
        *,
        ...
        if successor is not None:
            ...
            if succ is None:
                ...
                elsewhere = self.adapter.find_types(
    ...
    def reinstate(
        ...
        **Ruling R19: this covers edge FAMILIES and never edge instances.** An edge
        ...
        collisions, collision_why = self._alias_clash(
        ...
        for guard in order:
            if guard == "kind_mismatch":
                ...
                mine = (
                    ...
                    else {c.id for c in self._consumer_report(here).gates_on}
                )
    ...
    def merge_types(
        ...
        *,
        ...
        holders, clash_why = self._alias_clash(target_ns, right.name, right.kind, moving)
            ...
            if incoming:
                ...
                if incoming_kind == "action" and row.get("attributes"):
                    ...
                    for word in incoming:
                        ...
                        # have a column for.
                        named, named_why = self._word_rows(namespace, word)
                ...
                if breach is not None:
                    ...
                    continue
                clashes, clash_why = self._alias_clash(
    ...
    def _identity_closure(
        ...
        **It stops, and says so, rather than looping.** Three ways a chain can fail to
        ...
        alias_holders, alias_why = self._alias_map(ref.namespace, ref.kind, cache)
        ...
        for alias in aliases:
            ...
            for other in others:
                if other.name == name and other.kind == kind:
                    continue
                standing = self.adapter.get_type(namespace, name, kind=kind)
    ...
    def preflight(
        ...
        **And the gate is advisory by construction, which ACTIONS.md 4 states in the
        ...
        base = dict(
            ...
            warnings=tuple(
                self._retired_blast_radius(namespace, fam.effects, inputs)
            ),
        )
    ...
    def _observe_invocation_inputs(
    ...
    def record_invocation(
        ...
        **The TOCTOU gap is named rather than closed.** Between ``preflight`` returning
        ...
        self._observe_invocation_inputs(
    ...
    def invocations(
        ...
        **It does not page, and that is ruling R25/R47/R58's decision rather than this
        ...
        # filtered nothing: the shipped `DegradedAdapter` DROPS the family filter on such
