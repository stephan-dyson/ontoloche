...
| 2 | ~~**`docs/specs/PACKAGE.md` v0**~~ — **DONE 2026-08-28** ([`docs/specs/PACKAGE.md`](docs/specs/PACKAGE.md)) — `open_ontology` package shape, a **fifteen-primitive** storage-adapter protocol, SQLite + Postgres table shapes, the `attributes` schema-per-kind mechanism, and **109 contract tests as the definition of conformance** *(118 since row 3c)* | 2B needs `pip install` + Tenshen's own tables behind the adapter | Opus |
