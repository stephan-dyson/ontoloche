...
| 4b | **EDGES v0 implemented** — adapter primitives 16–18 on all three reference backends, `add_edge` / `retract_edge` / `neighbors` with the default assembly bound, `equivalent_to` seeded, edge conformance ids + broken-backend tests, the UC2/UC3 probes as tests *(sequenced ahead of #6 by the ordering rule: Tenshen slices 1–2 and beacon 21.2 need a real edge store, not only a spec)* | **Slices 1–2 (read seam, `relations` slot) and 21.2's edge backend** | Opus |
