...
| 2 | ~~**`docs/specs/PACKAGE.md` v0**~~ — **DONE 2026-08-28** ([`docs/specs/PACKAGE.md`](docs/specs/PACKAGE.md)) — `open_ontology` package shape, a **fifteen-primitive** storage-adapter protocol, SQLite + Postgres table shapes, the `attributes` schema-per-kind mechanism, and **109 contract tests as the definition of conformance** *(121 since row 3c)* | 2B needs `pip install` + Tenshen's own tables behind the adapter | Opus |
...
| 3c | ~~**Use-case validation pass**~~ — **DONE 2026-08-29** ([`docs/findings/3C-VALIDATION.md`](docs/findings/3C-VALIDATION.md)). UC3 (`uvpi-gqnh` DPR / `erm2-nwe9` 311-OTI / `693u-uax6` DOT) run against both specs, plus eleven adversarial rounds. **Mechanism 4's answer held** — three scoped `status` types coexist and `cross_namespace_merge` refuses non-overridably — and **seven contortions were recorded** (INTERFACE 8–12, PACKAGE B7–B8). Ruling **R4** landed; **Q1–Q7 are open for a ruling** (§6). The suite grew **109 → 119** and four correctness defects were fixed: `merge_types`' divergence guard was anti-correlated with its purpose; `retire()` read *"we could not check"* as *"nothing gates on this"*; **the kill row itself tripped** on Tenshen's declared capability shape; and a forced retirement nobody could audit went unrecorded | none directly — it is the kill-criterion mechanism finally exercised; every later spec inherits the protocol | Opus |
| 3d | **Upstream fixes from beacon 21.1** — U1 savepoint-scoped `transaction()` over a host-owned session (ruling **R5**, [`docs/decisions/2026-08-29-2b-upstream-findings-R5.md`](docs/decisions/2026-08-29-2b-upstream-findings-R5.md)) + reference adapter fix + borrowed-connection test; U2 a natively-degraded third reference leg in the conformance suite; U3 `attribute_projections`; U4 §3.3 doc sync + drift check; **+ R8** `consumers()` warns on an unregistered `gate`; **+ R12** per-run conformance coverage report | **the transaction seam beacon 21.2 builds on** — without U1, sharing a connection does not share a transaction | Opus |
...
| 3e | **v0.1 amendments from the 3c rulings** ([`docs/decisions/2026-08-29-3c-rulings-R6-R12.md`](docs/decisions/2026-08-29-3c-rulings-R6-R12.md)) — R6 `search_namespaces` on `resolve_type`; R10 name-level attribute schemas; R11 `reinstate` + `successor_active` reason; then the adversarial loop resumes on the amended surface | none directly — UC3/Phase 3 and governance; deliberately after EDGES so Tenshen's slices are not delayed | Opus |
