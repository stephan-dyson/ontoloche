...
| 3d | **Upstream fixes from beacon 21.1** — U1 savepoint-scoped `transaction()` over a host-owned session (ruling **R5**, [`docs/decisions/2026-08-29-2b-upstream-findings-R5.md`](docs/decisions/2026-08-29-2b-upstream-findings-R5.md)) + reference adapter fix + borrowed-connection test; U2 a natively-degraded third reference leg in the conformance suite; U3 `attribute_projections`; U4 §3.3 doc sync + drift check | **the transaction seam beacon 21.2 builds on** — without U1, sharing a connection does not share a transaction | Opus |
