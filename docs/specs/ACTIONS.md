...
> **Phase 3 has now specified it — ruling R60's one change, landed 2026-09-03 by row 7a.** Rule 2.4-9's *"anything else is not a precondition in v0"* stands for **this** document; what changes is that ACT4's missing mechanism has a home rather than a deferral. [`INGEST.md`](INGEST.md) §6 specifies one `Condition` for all four surfaces R60 named — twelve closed terms (ten operators over **one record's attribute values**, `all_of` / `any_of` composed by Kleene logic), **three-valued**, `why` required, declared on the entry, evaluated by the registry. **[Observed]**, that row's design test 3: read as *false*, an undecidable predicate excluded all 1,373 candidates and the loop proposed a new instance for one that exists; read as *true*, it leaked across both tenants. So Rule U at this shape is not a preference — it is the only reading that is neither of those.
>
> **The fifth precondition kind is still not taken and this is why the sequencing matters.** A `Condition` is not a `Precondition`: §2.4's four kinds are each *a question the registry can already answer with a call it already has*, and a value condition is answered by **reading the record**, not by calling anything. Whether that makes it a fifth `kind` or a separate field on the shape is the **build row's** to decide with an id behind it, and `INGEST.md` §9 keeps its own ids *planned* for the same reason. Recorded here in the same change because R60 requires the four surfaces to be amended together, and because a rule tagged `prose-only:` with no pointer is how a routed question stops being routed.
...
| 2.5-12 | An **input-determined** effect (rule 2.5-10, `namespace=None`) is judged for 2.5-11 over the namespaces the invocation's **own inputs** carry, never over the family's own — judging it in the family's namespace was a false answer in **both** directions *(row 6c, round 1)*. **Narrowed in round 3:** the candidate set is every namespace any input mentions, so an unrelated input dragged one in and the warning fired about a family live where the edge went. `record_invocation` judges it in the namespace the matching **observed** effect resolved to; `preflight`, where the landing namespace is unknowable, warns only when **every** candidate that has the family has retired it — otherwise Rule U's stated absence | `C19-83`, `C19-87` |
| 2.5-13 | A supplied `InputRef` whose **identity segments** cannot be written to the flat form of §2.3 — a `namespace`, `kind` or `name` carrying `:` or `#` — is refused `input_kind_mismatch` at **both** invocation doors, so the ledger's strings stay parseable by construction *(row 6c, round 1)*. **The refusal's `why` names which of the two failures this is** *(round 3)*: a segment carrying `#` writes a string `parse_ref` reads back as a **different reference**; a **namespace** carrying `:` writes one `parse_ref` **raises** on. Where the constraint should bind is **Q81** and is not this layer's to answer | `C19-82`, `C19-90` |
...
Invocation:
    ...
    reviewed_by:        str | None            # WHO reviewed it -- the actor on that
                                              #   event. The LAST reviewer; the whole
                                              #   sequence stays in `history`
...
| 6-12 | `invocations(unreviewed=True)` reads the **mode of record** (`Invocation.declared_policy`), never the family's mode today — an amendment must not move a historical invocation into or out of the queue *(row 6c, round 1)*. A row that carries **no** mode of record — filed before the field existed, or by a backend that keeps none — falls back to the live family, and `why_incomplete` **says how many rows did** *(round 3)*: rule 3-8's guarantee does not reach them, and a fallback nobody can see is a fallback nobody can act on | `C19-84`, `C19-89` |
| 6-13 | `review_invocation` **honours its `namespace`**: an invocation stored in another scope is `unknown_invocation`, not a silent cross-tenant drain; and an empty `reviewed_by` is a `ValueError`, as an empty `reason` is at `retire` *(row 6c, round 1)* | `C19-85` |
...
| 10-9 | A typo is an `order` naming groups **no family in THIS SCOPE carries, where the scope declared any surface at all** — the `namespace`-filtered pool, not the store-wide one. A host whose families all declare `reachability=()` gets an answer with zeroes, and keeps getting zeroes when a co-tenant registers a surfaced family; a `namespace` that holds no such family is a legitimate scope; a scope that *does* use surfaces still catches a misspelling *(ruling **R70**, row 6c, decided by §10's design test re-run over UC3's many-publishers catalogue)* | `C19-54`, `C19-74` |
| 10-10 | A group repeated in `order` is charged **once**, so `fits` and `would_evict` can never intersect | `C19-58` |
| 10-11 | The report **names the scope it answered over** (`ProjectionReport.namespace`), and an all-zero answer says **which** empty it is — a scope holding no `kind="action"` row at all, a scope whose rows are all retired, or a scope that simply declares no surfaces. Only the last is what rule 10-9 protects *(row 6c, round 1)*. **The census that tells them apart is its own read** *(round 3)*: `include_retired=True` and paged to exhaustion, with the selectable pool still active-only, so the retired case stopped being byte-identical to the typo'd one — and a scan that could not finish says *we could not read the scope*, never *the scope is empty* | `C19-86`, `C19-88` |
| 10-12 | A negative `budget`, a negative `reserved`, or a `reserved` larger than `budget` is a `ValueError` — a caller's mistake about its own arithmetic, answered as `retire` answers an empty `reason` | `C19-86` |
...
def projection(
    ...
    *,
    ...
    namespace: str | None = None,        # the SCOPE. `None` is the WHOLE STORE, and the
                                         #   report echoes what it answered over
) -> ProjectionReport | Refusal: ...
```
...
ProjectionReport:
    surface:          str
    namespace:        str | None            # the SCOPE this report answered over, verbatim.
                                            #   None = the caller supplied none and the
                                            #   answer is store-wide
    budget:           int
    reserved:         int
...
**Refuses with `action_family_unknown`** when `order` names a group **no family in this scope carries, and this scope declares surfaces at all** — a projection over an entirely unknown vocabulary is a typo, and an empty report for a typo is mechanism C committed by the call that exists to surface it. When `order` names a mix of known and unknown groups, the unknown ones appear in `counts` with `0` and `complete` goes `False` with a `why`, because a host legitimately assembles a context from groups that happen to be empty today.

> **The scope is the `namespace`-filtered pool, and getting that wrong cost a round in each direction** *(ruling **R70**, row 6c; rule 10-9)*. Round 1 of the spec row judged against the filtered pool with **no** *declares-any-surface* condition, and an empty namespace refused a real projection; the fix moved the judgement to the **store-wide** pool and added the condition — which fixed the empty namespace and opened **Q72**, because the store-wide pool makes one host's answer depend on an unrelated host's data. **[Observed, row 6c's design test over UC3's many-publishers catalogue]** four `ingest_dataset_*` families in the `dpr` namespace, alone on the store: `counts={"catalogue_ingest": 0}`. One `close_311_request` registered by a co-tenant in `oti_311`, with nothing about `dpr` changed: **`Refusal(action_family_unknown)`** on the same call. UC3 is dozens of publishers in one catalogue, so *"somebody else's namespace"* is the ordinary condition rather than the exotic one.
>
> The scoped pool answers both readings at once: an empty namespace declares no surface and gets zeroes; a co-tenant's surface is out of scope and cannot make a neighbour's ordinary projection a typo; and a scope that *does* use surfaces still catches a misspelling, which is the whole of the rule. The pool judged is the **active, declared** one `counts` is computed over — a scope whose only surfaced family is *retired* answers with zeroes rather than refusing, which is this project's standing direction: never turn *we could not find it* into a refusal.
...
| T3.5 | the **shipped** `merge_types` still refuses | `Refusal(reason="cross_namespace_merge")` with the `equivalent_to` **and** the `reconciled_with` edge present — **and again** under `acknowledge=["cross_namespace_merge","definitions_diverge"]` | **PASS — the load-bearing check, against `ontoloche.Registry`** |
