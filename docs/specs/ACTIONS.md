...
| 2.5-12 | An **input-determined** effect (rule 2.5-10, `namespace=None`) is judged for 2.5-11 over the namespaces the invocation's **own inputs** carry, never over the family's own — judging it in the family's namespace was a false answer in **both** directions *(row 6c, round 1)* | `C19-83` |
| 2.5-13 | A supplied `InputRef` whose **identity segments** cannot be written to the flat form of §2.3 — a `namespace`, `kind` or `name` carrying `:` or `#` — is refused `input_kind_mismatch` at **both** invocation doors, so the ledger's strings stay parseable by construction *(row 6c, round 1)* | `C19-82` |
...
| 6-12 | `invocations(unreviewed=True)` reads the **mode of record** (`Invocation.declared_policy`), never the family's mode today — an amendment must not move a historical invocation into or out of the queue *(row 6c, round 1)* | `C19-84` |
...
| 10-11 | The report **names the scope it answered over** (`ProjectionReport.namespace`), and an all-zero answer says **which** empty it is — a scope holding no `kind="action"` row at all, a scope whose rows are all retired or still proposed, or a scope that simply declares no surfaces. Only the last is what rule 10-9 protects *(row 6c, round 1)* | `C19-86` |
