...
**`warnings` vocabulary, complete — seventeen values across four carriers** *(eleven at row 3c/3d; five added by [`EDGES.md`](EDGES.md) v0, row #4, 2026-08-29; one by ruling **R11**, row 3e)*:
...
**What `reinstate` does NOT do.** It does not restore anything a merge did: `merge_types` writes `from_`'s name into `into`'s `aliases` and reinstating `from_` **does not remove that alias**, so the merged word keeps resolving to the survivor as well. That case is normally caught by refusal #1 — a merge sets the survivor as `successor` and the survivor is active — and where it is not (the survivor was later retired), the alias stays and is visible on `into`. Removing it would be a second destructive act hidden inside this one. It also does not resurrect anything outside the registry: v0 holds no instances, so there is nothing else to bring back (§1).
