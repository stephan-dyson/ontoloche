...
**`action_declarations_diverge` — the thirty-third, and §5.10's refusal #2 has no operand for an action family.** Returned by every door that COLLAPSES two `kind="action"` families onto one word — `merge_types`, `retire(successor=)` and `import_types`' alias write — when their **governance declarations** contradict. The compared keys are `approval_mode`, `min_auto_tier`, `reversibility` and `effects`: the blast radius. `inputs`, `preconditions`, `reachability` and `payload_schema` are the family's **shape** and are deliberately not compared — a shape divergence does not let a caller invoke something they could not invoke before.
