...
**Ruling R11 reserves a further value.** `successor_active` is specified by [`decisions/2026-08-29-3c-rulings-R6-R12.md`](../decisions/2026-08-29-3c-rulings-R6-R12.md) R11 and lands with `reinstate` in **row 3e**, which will make **nineteen**. It is named here so the count is reconcilable, and it is deliberately *not* in the list above, because R3's rule is that a value is added in the change that introduces it — and nothing introduces it yet.
