...
| **5.3.2-9** | **The confidence a stale redirect carries is `min(resolver_score, J)`**, where `J` is the **Jaccard agreement of the two written extents**, `|L ∩ R| / |L ∪ R|`. `L` and `R` are the sets rule 5.3.2-1 already reads, so **no new read is added to produce the number**. A still-agreeing pair gives `J = 1.0` and rule 5.3.2-4's unchanged 1.0 falls out of the formula rather than being special-cased. **Containment (`|L ∩ R| / |L|`) is NOT used**: it scores mere growth at 1.0 and would hide it, which is the same objection this section already makes to containment for the warning | `C3-20` |
| **5.3.2-10** | **`kind="action"` is verified too, by its OWN operand.** An action family has no extent — §5.10's refusal #2 is skipped for actions by design ([`ACTIONS.md`](ACTIONS.md) §2.1) — so what its identity claim stands on is its **governance declaration**, and this call compares the same keys the write doors compare (`_action_declarations_diverge`). A contradiction carries `identity_stale`, the **same** value rather than a minted variant, because it names the same fact: *this identity claim no longer holds*. Ruling **R71**'s precedent — a value gains carriers rather than growing a twin | `C3-21` |
