...
- **`existing`** — a registered type fits. `confidence` is set — **except on a redirect whose identity claim could not be SCORED, where it is `None`** (Rule U; ruling **R99**, rules 5.3.2-9 and 5.3.2-13(d)). A caller doing arithmetic on `confidence` without a `None` check faults on that answer, and it is called out here rather than only in the redirect prose below because this list is what a caller skims.
...
**A redirect whose identity claim has gone STALE still answers, SAYS SO, and NO LONGER ANSWERS AT 1.0** *(the **Q56 default**, row 4d, 2026-08-30; the expensive half taken by founder ruling **R99**, row 6f, 2026-09-09)*. When an **exact** hit is answered through an **alias** or a **successor**, this call **re-verifies the identity claim it is about to make** — for `kind="predicate"` by re-reading both extents, for `kind="action"` by comparing both governance declarations (§5.3.2's rules say which fact per kind) — paged to exhaustion, a truncated read folded into *unknowable*, exactly as `merge_types`' own refusal #2 does. If the claim no longer demonstrably holds, the returned `TypeEntry` carries `identity_stale`, `reason` says which claim went stale, **and the confidence is lowered to how much of the claim still holds.**

> **What this section promises now, stated exactly, because the old sentence promised something else.** Until 2026-09-09 this section promised **`confidence: 1.0` on every `existing` outcome, unconditionally**, and said so. It no longer does. **What it promises instead:**
>
> 1. **`outcome` and `type` are unchanged.** A redirect still resolves, and it still resolves to the survivor. §5.10's *"the old word still resolves"* is intact, and **this call never refuses on its own account** — see rule 5.3.2-11 for why, and for what replaced refusing.
> 2. **`confidence: 1.0` means the registry vouches for the identity**, and it is now a claim the call has verified at the read rather than one it inherited from a write.
> 3. **A confidence below 1.0 means the answer is correct and the vouching is withdrawn** — the caller is being handed the survivor together with a measure of how much of the identity claim justifying it still holds.
> 4. **`confidence: None` means the registry could not score the claim at all** (Rule U), not that it scored zero.
> 5. **`min_confidence` now governs this path**, which it did not before (rule 5.3.2-12). A caller that declines to act on a weakened identity says so with its own bar and receives `none`; a caller that does not care is unaffected.
>
> **Ruling R99's own words for what it cost:** *"A shipped guarantee changes… Every caller that reads 1.0 as 'safe to act' gets a new answer shape."* Beacon slice 1 was told on 2026-08-30 that it *"can trust a 1.0 redirect, or is told not to."* **This is the second branch.**
...
>
> **Refusing to answer, or answering below 1.0, would change the guarantee this section makes**, and deciding what this registry declines to serve is not an implementation call. It was **Q56**, it was the founder's, and it was open for ten days and twenty-three kill-row trips. **It is RULED: `read`, 2026-09-09, [R99](../decisions/2026-09-09-founder-ruling-R99.md).** The claim is verified where it is MADE, and this call may act on what the verification found. Row 4d shipped the cheap half — the fact *reported*, never *suppressed*; **row 6f ships the response**, and the paragraph above is what the guarantee says now. The comparison is between the two **written words** and never their identity closures: after ruling **R54** `_extent` can resolve an identity, and asking whether one identity equals itself is circular, so this call asks whether the two words still denote the same set **of their own accord**. Non-predicate hits read no extent and pay nothing. `C3-14`, `C10-14`.
...
#### 5.3.2 Staleness at the read — rules *(the **Q56 default**, row 4d; **rules 9–13 by founder ruling [R99](../decisions/2026-09-09-founder-ruling-R99.md), row 6f, 2026-09-09**)*

Standing constraint 8 / ruling **R31**. **This table enumerates the rules rows 4d and 6f added to §5.3 and no others**; §5.3.1's nine rules are ruling R6's and predate the constraint. **Rules 1–8 are row 4d's cheap half — the detection. Rules 9–13 are row 6f's expensive half — the response**, and 5.3.2-3 is struck rather than removed because it is what the registry promised for the ten days in between.
...
| 5.3.2-3 | ~~`outcome`, `type` and `confidence: 1.0` are **unchanged**~~ — **SUPERSEDED by R99 and rules 5.3.2-9 to 5.3.2-13 below.** `outcome` and `type` are still unchanged; **the confidence is not.** The rule is kept struck rather than deleted because it is what the registry promised from 2026-08-30 to 2026-09-09 and a caller may have been built against it | `C3-14`, `C10-14` |
| 5.3.2-4 | A still-agreeing pair carries **no** warning, and **this call adds nothing to the confidence it would otherwise have answered**. *(Amended row 6f round 2: the rule used to say it **answers at 1.0**, full stop. **[Observed]** that is false on the ALIAS door under a deployment supplying its own resolver — `PACKAGE.md` §2.6's production path — where the confidence is the resolver's own score: a still-agreeing pair scored `0.92` answers `0.92`. The **successor** door is registry-computed and unaffected. The old wording was true only of the shipped `DeterministicResolver`, which is the mistake `C3-11`'s own rationale exists to name.)* A hit where either side is not a predicate reads **no extent** at all — but it is **no longer unverified**: see 5.3.2-10, which gives `kind="action"` its own operand. A kind with neither operand is verified by neither and answers at 1.0, and 5.3.2-13 records that as a residual rather than as coverage | `C3-14`, `C10-14` |
...
| **5.3.2-9** | **The confidence a stale redirect carries is `min(resolver_score, J)`**, where `J` is the **Jaccard agreement of the two written extents**, `|L ∩ R| / |L ∪ R|`. `L` and `R` are the sets rule 5.3.2-1 already reads, so **no new read is added to produce the number**. A still-agreeing pair gives `J = 1.0` and rule 5.3.2-4's unchanged 1.0 falls out of the formula rather than being special-cased. **Containment (`|L ∩ R| / |L|`) is NOT used**: it scores mere growth at 1.0 and would hide it, which is the same objection this section already makes to containment for the warning. **This rule is the AGREEMENT half only** — the composition with the resolver's score is **5.3.2-14**, the unknowable read is **5.3.2-15**, and the action operand's key set is **5.3.2-16**. Each carries exactly ONE enforcement status, so a reader can tell which clause is pinned and which is merely asserted | `C3-20` |
| **5.3.2-10** | **`kind="action"` is verified too, by its OWN operand.** An action family has no extent — §5.10's refusal #2 is skipped for actions by design ([`ACTIONS.md`](ACTIONS.md) §2.1) — so what its identity claim stands on is its **governance declaration**. **This call compares ALL EIGHT of `ACTIONS.md` §2.2's declared keys.** **`Q99` is now RULED `all eight` ([R102](../decisions/2026-09-09-founder-ruling-R102.md), row 6g), so the write doors compare the same eight and the asymmetry this rule described is DISSOLVED** — both sides take the same evidence, the read **scores** and never refuses (5.3.2-11), the write **refuses** a contradiction. *"May these be joined?"* and *"do these still denote one thing?"* remain different questions with one operand. **The superseded sentence, kept because it was the shipped reasoning for one day:** *"a strict SUPERSET of the four `_GOVERNANCE_KEYS` the write doors compare, and the asymmetry is deliberate: the write doors decide what to FORBID, which is a policy question and is Q99, while this call decides what to VOUCH FOR."* **The two sides still part company on ABSENCE, and a caller must not be surprised by it:** a key present on one side and absent on the other does **not agree** here and so lowers the score, while the write door does **not** read that as a contradiction on the four keys R102 added — R102 §4 leaves absence unruled and row 6g took the least-refusing option rather than minting a refusal nobody decided (`C19-106`). **So two families that PASS the merge door can still redirect below 1.0 here** — `C3-21`'s fixture is exactly that. Comparing lists is order-insensitive (`effects` through `effect_identity`, the rest as sets) because §1's non-goals say *"no ordering"*, and because row 6d's round 3 found the opposite comparison **closing a legal operation**. A difference carries `identity_stale`, the **same** value rather than a minted variant, because it names the same fact: *this identity claim no longer holds*. Ruling **R71**'s precedent — a value gains carriers rather than growing a twin | `C3-21` |
| **5.3.2-11** | **This call does NOT refuse on its own account, and R99's authorisation to refuse is deliberately unused.** In every state reachable by ordinary calls the registry **can** name the correct answer — §5.10 promises the old word still resolves and the survivor is the identity it belongs to — so refusing would withhold a correct answer. Refusing on an *unknowable* read would additionally ban this door on a legal declared-degraded backend, which is `C10-09`'s, `C3-13`'s and `C12-13`'s lesson three times over. **What replaces refusing is rule 5.3.2-12** | `C3-22` |
| **5.3.2-12** | **`min_confidence` governs this path**, so a caller declines with its own bar rather than the registry declining for everyone. **[Observed, row 6f]** it did not: the exact-hit redirect returned before reaching the test (`registry.py:1707` sits after the return at `1705`), and `min_confidence=2.0` still answered `existing` at 1.0. That was invisible while the redirect always answered 1.0 and goes live the moment 5.3.2-9 lowers it. Below the bar this call answers `none` with `alternatives` populated — **§5.3's own stated rule, applied to the one path that skipped it**, not a new policy | `C3-23` |
| **5.3.2-13** | **The residuals, stated as residuals and not as coverage.** (a) A **transferred word** reaches the survivor by neither an exact alias nor a successor, so it is verified by neither operand — the blindness trips 12 and 13 proved of the cheap half is **INHERITED, not narrowed**, and R99 §4 named it. (b) A kind with **neither** operand (`entity`, `value_set`) reads nothing and answers 1.0, exactly as before. (c) The extent comparison is over **raw member names** while the resolver scores `_norm(candidate)`, so it can over-report; harmless as a warning, and a lowered score is a real cost. **(e) A PAGE-CAPPED backend changes the ANSWER, not just the warning** — a large extent it cannot page to exhaustion is *unknowable*, so 5.3.2-15 applies and any positive `min_confidence` becomes an effective decline on that backend while an uncapped reader of the same store answers `existing`. That is a real production shape, not only a test double. **(f) `stores_aliases=False` is covered by (a) too** — the alias gate requires a matched alias and such a backend reports none, so the alias door is never verified there. **(d) On a backend declining `stores_attributes`, EVERY action redirect reports `identity_stale` with `confidence: None`** — both declarations read back empty, so *unknowable*, and the call cannot tell that from a genuine contradiction. Rule-U-consistent and **total on such a leg**, which is broader than (a)–(c) and is stated rather than left for a deployment to discover | `prose-only:` each clause names a state the change does **not** reach, and a contract id asserting an absence of behaviour would pass on a registry that never implemented the feature at all — 5.3.2-5's own reason for a tag. (a) and (c) are carried into the next row's brief rather than closed here |
| **5.3.2-14** | **The resolver's own score and the identity's agreement compose by `min`**, and a `None` on either side **absorbs** rather than being dropped. The answer is no more trustworthy than the weaker of *does the word match* and *does the identity still hold*. *(Row 6f first tagged this `prose-only:` on the claim that the resolver's score is **always 1.0** on a stale redirect and the `min` therefore unobservable. **That claim was FALSE and the tag is withdrawn.** A round-2 lens built a legal custom resolver and disproved it; re-measured across a range, the composition is reached for scores **0.9 through 1.0**, and with a high-agreement pair — `J = 0.9524` against a score of `0.94` — the **score** is the weaker half and the call answers `0.94`. The original instrumentation tested ONE score, found no call, and generalised.)* | `C3-27` |
| **5.3.2-15** | **An UNKNOWABLE read answers `confidence: None`** — not `0.0`, and not a refusal. Rule U at the confidence field, in this section's own words: *"`None` means 'did not score', NOT zero."* 5.3.2-9's formula needs `J` and an unknowable read produces none, so the one cell where the formula has no input gets its own rule instead of being left to prose. **A consequence stated as a property rather than left emergent:** the same store read through a page-capped backend and an uncapped one yields a **different** confidence for the same identity claim, and under a positive `min_confidence` a **different outcome** — `existing` uncapped, `none` capped. This confidence measures **what THIS READER could establish**, not the identity alone, and two deployments comparing confidences are comparing two different questions | `C3-28` |
| **5.3.2-16** | **The action operand compares `ACTIONS.md` §2.2's EIGHT keys by NAME, and a key PRESENT on one side and ABSENT on the other does not agree.** *(Row 6f round 2, and it was a defect from `E`'s own **family** -- *a confident answer standing in for a fact the system had or could not have*, 5.3's own name for it -- rather than `E` itself: the first cut derived the key set from the union of the two stored dicts and compared `mine.get(key)` against `theirs.get(key)`, so a family positively declaring `reachability=[]` scored **identical** to one that never declared the key — both flatten to the same empty set — and the pair answered a **clean 1.0 with no warning**. §2.2 is explicit that the distinction is load-bearing: an empty list is a positive declaration, not a forgotten field. And `family_declaration_problem` requires only `reversibility` and `approval_mode` to be present, so a legal schema-valid family may carry none of the other six.)* A key absent from **both** sides is not a disagreement — neither family spoke | `C3-29` |

> **`identity_stale` is permanent by construction once a merged identity accretes members, and a consumer should read it that way** *(row 4d, round 2; **one premise corrected by row 6f, 2026-09-09, after it was measured rather than reasoned about**)*.
>
> > **CORRECTION, in place and visible.** This note used to say: *"The absorbed word is retired, so its own written extent can never grow again; the survivor's can."* **The first clause is FALSE.** **[Observed, row 6f]** after a legal join, `propose_type("late_declarer", …, predicates=["pred_a"])` naming the **retired** predicate is accepted and approved **by ordinary calls**, and the absorbed word's written extent moves from `{shared}` to `{late_declarer, shared}` while the survivor's stays `{shared}`. Retiring a word makes the **name** unusable (§5.9); it does **not** stop a new type declaring that word as one of its `predicates`. Reproduced by [`readside_t1_probe.py`](../tools/readside_t1_probe.py).
> >
> > **What the note can still claim without that clause, which is less than it claimed.** The conclusion — that `identity_stale` is effectively permanent — survives, because no call in this document amends a live type's `predicates` to *remove* a member, so neither extent can shrink and an inequality once reached cannot be undone by ordinary curation. **What does NOT survive is the direction.** The old clause implied the absorbed word's extent could only ever be a **subset** of the survivor's, so the only reachable drift was the survivor growing. It is not: the absorbed word's extent can grow independently, so a pair can reach a state where **each side holds a member the other lacks** — the two words demonstrably denoting different sets — and that state is reachable with ordinary calls. Rule 5.3.2-9 scores it lower than mere growth precisely because it is a different and worse fact, and the old clause is why nobody had asked whether it was reachable. That is not a bug in the comparison — it is the write-time guard's own strictness read at the read, and **weakening it to containment would make the warning miss Door 1**, the sixth trip's own walk, where the absorbed word's extent is a subset of the survivor's and the pair is refused non-overridably anyway. What it means is that the value says *this identity has grown apart from the equality that justified it*, not *act on this now*. Whether that is the right shape for a consumer is **Q66**.
...
| `retire_no_op:already_retired` | **`TypeEntry` only** — `retire` on a row that is **already retired**. Never a refusal: nothing was prevented and the desired state already holds, which is ruling **R4**'s own line for a call that would otherwise quietly do nothing, and it is `reinstate_no_op:not_retired`'s treatment of the identical question one call along. **The kill row's TWELFTH trip is why it exists** *(row 6c)*: this call read `.status` twice and both times on the **successor**, so a repeat retirement toward a second successor copied the retired row's aliases onto a **second live row while the first still held them** — two active entries answering to one word, which is `C16-06` and mechanism **4**. It is also the more correct answer to what the old behaviour was answering badly: rewriting a standing tombstone's `retire_reason`, `retired_by`, `retired_at` and `successor` is an **edit** of a provenance-bearing row, and §5.8's rule is that *a correction is a new event, never an edit*. The path to change a successor is **`retire` the successor first, then `reinstate`, then `retire`** — *(corrected in row 6c's round 3: the first cut of this row said "`reinstate` then `retire`", and `reinstate` refuses `successor_active` **non-overridably** while the successor is live, which is the ordinary case. `successor_active.detail["path_back"]` had the correct step one all along. A remedy nobody can run is an unreachable guard written in prose.)* `C9-29` |
...
| `identity_guard_skipped:<guard>:<capability>` | **`TypeEntry`, `MergeResult` and `Refusal`** — every door that asks §5.10's identity guards: `propose_type`, `approve`, `import_types`, `retire`, `merge_types`, `reinstate`. **An identity guard that could not be EVALUATED because the backend declines a capability it needs.** Refusal **#1** compares CONSUMER SETS, and on `indexes_membership=False` a stored row's `predicates` come back empty, so both sides read blank and #1 compares equal **vacuously**. The comparison is deliberately unchanged — refusing would ban these doors on UC1's own declared shape, which is `C10-09`'s lesson, `C3-13`'s and `C12-13`'s — but the caller is now **told**. **Not `alias_check_incomplete`**, which says *a LOOK did not finish and there is more to read*; this says *a GUARD was never asked, and the store does not hold the fact at all*. One word for two facts is §2.3's Cause B. **And it rides a `Refusal` too** *(row 6d, round 2; findings G4 / A14 / K5 / X11, four lenses)* — until then every door dropped its accumulator the moment it refused, so this value reached a caller **only when the call succeeded**. That is the half that matters more: a caller told *no* could not tell *we checked and the answer is no* from *we could not check and the answer is no anyway*. `C10-27`. **The kill row's NINETEENTH trip** (row 6d, ruling **R92**): the same five calls refused `different_consumer_sets` **non-overridably** on sqlite and were written with `warnings=()` on the degraded adapter, with `resolve_type` answering at **1.0**. `C12-24` |
| `import_field_ignored:<field>:<value>` | **`TypeEntry` only** — `import_types`. A key the row supplied that this call does **not** honour. `import_types` takes ONE `namespace` for the whole batch (§2.5's Foundry mapping is a per-call scope, not a per-row one), and a row carrying its own `namespace` had that key **silently dropped**: the identity was written into the CALLER's scope with `warnings=()`, so a dump with a namespace column landed its rows in the wrong place with nothing said. **Not a refusal** — the write is legal and the mapping is the documented one; what was missing is that it says so. Accepted-and-ignored is the `mark_reviewed` shape row 6c fixed one call along. Row 6d, finding **X6**. **And it survives the row being DECLINED** *(round 2, finding X11)*: `_refused_import` returned `import_refused:<reason>` alone, so every note this call had collected on the way to declining the row — this value and the `alias_check_incomplete` values beside it — was dropped by the `continue` that followed. The accumulator is filled at five sites and was read at exactly one, the success path. `C12-27` |
...
| `edge_family_retired:<name>` | **`NeighborReport`**, **`Edge`**, and — from row 6c — **`Preflight`** and **`Invocation`** | `EDGES.md` §4.3. A retired family's edges were never deleted, so it is still searched; `add_edge` onto a retired family is not refused and the returned `Edge` says so. **Ruling R71** adds the two ACTIONS carriers for the identical fact one layer up: a family's declared `kind="edge"` blast-radius family is retired **at invocation time**, which [`ACTIONS.md`](ACTIONS.md) §2.5-7's *"the door is the declaration"* had no answer for. **No value minted** — reused, per §2.3's discipline — and **both** invocation doors carry it, because one of two call sites is this project's most-repeated defect. `C19-75`, `C19-76` |
...
| `aliases_transferred:<successor>` | **`TypeEntry` only** — the tombstone `retire(successor=)` returns | §5.9, ruling **R75**, row 6c's THIRD adversarial round. The call transferred the retired row's aliases onto its successor, and this names the row that answers to them now. **R75's write lands on the SUCCESSOR and the call returns the TOMBSTONE**, so the one act that moved a word from one identity to another was invisible at the call performing it: the caller got back an entry still carrying the retired row's own aliases and nothing saying a second row had begun answering to them. The `aliases_transferred` **event** is on the survivor — the right home for the history and the wrong one for the caller, because reading it means already knowing to look. A warning and never a refusal: nothing was prevented, and the write is the one R75 exists to perform. `C9-33` |
| `aliases_removed:<word>[,<word>…]` | **`TypeEntry`** — the entry `import_types` returns | §5.4/§5.9, row 6c's first and second adversarial rounds, closed in the third. An import writes `aliases` **wholesale**, so every word the standing row answered to and the dump does not name is erased. **[Observed]** a `retire(successor=)` transferred `zeta` onto `beta` and filed `aliases_transferred`; an ordinary `import_types([{"name": "beta", "aliases": []}])` then left `beta.aliases == ()` with no warning at all — and the transfer's own event **still standing, asserting a fact the store no longer held**. A word that resolved at confidence 1.0 stopped resolving, and §5.3 calls that confidence a guarantee. Not a refusal: removing an alias is a legitimate act and an import is a vocabulary arriving *already decided*. What it owes is what §5.8 asks of any correction — **say it, and record it as a new event rather than as an edit** — so the same change files an `aliases_removed` event beside the warning. `C12-19` |
| `word_previously_retired:<holder>` | **`TypeEntry` only** — the retired holder `propose_type`, `approve` and `import_types` hand back; nothing is created (§5.9) | §5.4/§5.9, **the kill row's FOURTEENTH trip**, row 6c's third adversarial round. The name being minted is a word a **RETIRED** row still answers to as an **alias**, and this names that row. The eighth trip closed the retired half of the **name** door (`_word_rows`); the retired half of the **alias** door was asked by nothing, because `_alias_holder` and `_alias_clash` read active rows only by design. **[Observed, five ordinary calls, no merge, no `force`, no acknowledgement]** a tombstone keeping `('zeta',)` by §5.8's design, `propose_type("zeta")` accepted **with no warning at all** — and the store then refusing **both** `merge_types` (`predicate_merge`) and `reinstate` (`alias_collision`) non-overridably, so the tombstone can **never be brought back** and ruling **R11**'s governance act is permanently unavailable. Its own value and not `name_previously_retired`: *this word WAS a retired row's name* and *a retired row still ANSWERS TO this word* are two facts, and one word for two facts is §2.3's Cause B. `C4-15`, `C5-13`, `C12-21` |
...
Refusal:
    ...
    detail:   dict
    warnings: list[str]      # §5.4's vocabulary -- what the call could NOT check on its way to refusing
...
- **And a retired row's ALIASES are not reusable either, which is the same rule and took until the kill row's FOURTEENTH trip to state** *(row 6c, round 3)*. `propose_type`, `approve` and `import_types` all return the retired **holder** with `warnings: ["word_previously_retired:<holder>"]` and create nothing. §5.8 keeps a tombstone's words by design, so they are an **unconsumed permission** — standing rule (c) — and the register had been applying that rule to `name` alone: `_alias_holder` and `_alias_clash` read **active** rows by design, and `_word_rows` compared **names** only, which left one quadrant of the 2×2 open. **[Observed]** the store that results refuses `merge_types` *and* `reinstate` non-overridably, so the tombstone can never come back. `C4-15`, `C5-13`, `C12-21`; [`check_merge_guard.py`](../tools/check_merge_guard.py) gains a **tenth axis** driving all three mint doors at a tombstone's alias, proved by mutation.
...
### 5.12 `Refusal.reason` is a closed vocabulary *(added by ruling R3, 2026-08-28; fifteenth value added by ruling R4, row 3c, 2026-08-28; four EDGES values added by row #4, 2026-08-29; **twentieth and twenty-first** added by ruling R11, row 3e, 2026-08-29; **six ACTIONS values added by row #6**, 2026-08-29; **a seventh by that row's first adversarial round**; **a thirty-first — `unknown_invocation` — by ruling R73, row 6c, 2026-09-01, in the change that specifies the call which made it reachable**)*

A project whose thesis is that governed vocabularies resist rot does not ship an open-ended `reason` string in its own contract. `Refusal.reason` takes exactly these **thirty-three** values — eleven defined in this document, three introduced by [`PACKAGE.md`](PACKAGE.md) v0 and adopted here, one added by ruling **R4**, four by [`EDGES.md`](EDGES.md) v0, two by ruling **R11**, seven by [`ACTIONS.md`](ACTIONS.md) v0, one by row 4d's first adversarial round, one by its third, **one by ruling R73 in row 6c**, **one by ruling R91 in row 6d** — the transfer half of the fifteenth and sixteenth kill-row trips — and **one more by row 6d's finding A3**:

`different_consumer_sets` · `predicate_merge` · `kind_mismatch` · `cross_namespace_merge` · `retired_operand` · `definitions_diverge` · `no_consumer_evidence` · `live_consumers` · `tier_below_auto_approve_policy` · `already_decided` · `unknown_proposal` · `proposals_not_stored` · `cannot_record_override` · `attributes_schema_violation` · **`consumer_source_read_only`** · **`edge_family_unknown`** · **`endpoint_kind_mismatch`** · **`edge_store_absent`** · **`unknown_edge`** · **`successor_active`** · **`alias_collision`** · **`action_family_unknown`** · **`precondition_unmet`** · **`human_approval_required`** · **`tier_below_action_policy`** · **`effect_not_permitted`** · **`action_store_absent`** · **`input_kind_mismatch`** · **`successor_unregistered`** · **`successor_is_self`** · **`unknown_invocation`** · **`word_held_by_tombstone`** · **`action_declarations_diverge`**

**`unknown_invocation` — the thirty-first, and it is the one [`ACTIONS.md`](ACTIONS.md) §7 argued for and DECLINED.** Returned by `review_invocation` ([`ACTIONS.md`](ACTIONS.md) §6.5) when no invocation is stored under the given id. §7's argument for declining it was **explicitly conditional** — *"no call in this document names an existing invocation by id"* — and §6.5's fifth call, which row 6b found the specification needed and raised as deviation **D-6b-3** rather than taking on its own authority, names one. **Ruling R73** mints the value in the same change that specifies the call, which is R3's rule applied to a condition that expired rather than to a mind that was changed.

> **Not `action_family_unknown`, and the build row's reuse of it was recorded as a mismatch rather than defended.** That value names a missing **family**; this names a missing **invocation**. One word for two objects is §2.3's Cause B — the same argument that keeps `unknown_edge` separate from `edge_family_unknown` — and it is not abstract here: a host draining a review queue and told *no such action family* would go looking for a family that is registered, live, and not the problem. `C19-80`.
...
**`action_declarations_diverge` — the thirty-third, and §5.10's refusal #2 has no operand for an action family.** Returned by every door that COLLAPSES two `kind="action"` families onto one word — `merge_types`, `retire(successor=)` and `import_types`' alias write — when their **governance declarations** contradict.

> **AMENDED by founder ruling [R102](../decisions/2026-09-09-founder-ruling-R102.md), `Q99` ruled `all eight`** *(row 6g, 2026-09-09)*. **The compared keys are ALL EIGHT of [`ACTIONS.md`](ACTIONS.md) §2.2's declared keys.** Every key an action family declares is part of the identity two families must share before they may collapse.
>
> **The superseded sentence is kept rather than deleted**, because it is what the registry promised between 2026-09-05 and 2026-09-09 and a caller may have been built against it: *"The compared keys are `approval_mode`, `min_auto_tier`, `reversibility` and `effects`: the blast radius. `inputs`, `preconditions`, `reachability` and `payload_schema` are the family's **shape** and are deliberately not compared — a shape divergence does not let a caller invoke something they could not invoke before."*
>
> **Why it was wrong, in one measured sentence.** Two families agreeing on those four and differing on `preconditions` collapsed with **no refusal, no `force` and no acknowledgement**; `resolve_type` then answered the dead word with the survivor at **1.0**, a Haiku-tier actor recorded `applied`, and the survivor's ledger read `n=0` — the record filed under the dead word. **[Observed]** by row 6f, re-run independently by the supervisor, and re-run again at the write door by row 6g's [census](../tools/writeside_a3_census.py). That is the route by which entry `A3` fires the governance register's stop criterion ([R100](../decisions/2026-09-09-founder-ruling-R100.md)). **The comparator could not see the contradiction it exists to catch.**
>
> **How the four list-valued keys are compared, and this clause is load-bearing.** `effects` through `effect_identity`, and `inputs`, `preconditions` and `reachability` as **SETS** — the same `_UNORDERED_DECLARED_KEYS` the read side uses, not a second opinion. §1's non-goals say *"no ordering"*, and row 6d's own A3 fix compared the list-valued `effects` with `!=` and **CLOSED A LEGAL OPERATION** non-overridably at all three doors (`C19-100`). Widening from four keys to eight adds **three more list-valued keys**, so `!=` would have re-created that defect three more times. `C19-105`.
>
> **What R102 did NOT decide, and what the code therefore does.** *"`all eight` says which keys are compared WHEN BOTH SIDES DECLARE, not what to do when one does not."* So on the four keys R102 added, a key **present on one side and absent on the other is NOT a contradiction** — the least-refusing option, consistent with this door's own declare-nothing branch. `C19-106`. **On the four keys already compared, a per-key absence DOES refuse**, non-overridably, and has since `304967a`; that behaviour is unchanged here and is recorded as an open question rather than settled. Per-key **severity** — whether a `payload_schema` contradiction deserves the same non-overridable refusal as an `approval_mode` one — is likewise **not ruled**.

Refusal #2 is skipped for actions **by design** ([`ACTIONS.md`](ACTIONS.md) §2.1 argues that actions must be mergeable) and **nothing was put in its place for what an action family actually is.** So `resolve_type` answered the dead word with the survivor at **1.0** while `preflight` answered the same word with the **tombstone's** policy, and a Haiku-tier actor recorded `applied` against a verb the survivor declares human-approval-only and irreversible. **One word, one identity, two governance answers.**

> **Non-overridable, and §2.2's own cross-field rule is the argument.** A family declaring `reversibility="irreversible"` with `approval_mode="auto"` is refused **at declaration** with `attributes_schema_violation`, which no acknowledgement moves. A door that produces the same contradiction by **collapse** must refuse it the same way, or the rule has a door in it — the THIRD trip's own sentence, where a pair refused under every acknowledgement collapsed through `retire(successor=)`.

> **What it does NOT decide.** Whether *one word, two governance answers* is itself a kill-row identity failure is **Q94**, the founder's, minted by ruling **R91**. This closes the BLOCKING without answering it: two families that AGREE still merge, which is the narrowing a careless fix deletes.

`C19-97`, `C19-98`, `C19-99`.

**`word_held_by_tombstone` — the thirty-second, and the kill row's FIFTEENTH and SIXTEENTH trips are why.** Returned by the doors that **MOVE** a word onto a live row — `merge_types`' word move, `retire(successor=)`'s R75 transfer, `import_types`' alias write, and **`reinstate`, which moves a word onto a live row by making the row live** — when a **RETIRED** row still answers to that word **as one of its ALIASES**, of any kind. **Non-overridable**: `force` overrides what could be SEEN, never what would become TRUE.

> **`reinstate` was added to this list by the kill row's TWENTY-THIRD trip** *(row 6d, round 3; ruling **R94**)*, and the sentence above had named **three** doors where the surface has **four** since the value was minted. **[Observed]** in four ordinary calls with no `force`: `reinstate` returned a `TypeEntry` with `warnings=()`, `resolve_type` answered the contested word at **1.0**, and the other tombstone was then refused `alias_collision` non-overridably — the permanent un-reinstatability this entry's own next paragraph names as the harm. `C9-37`.
>
> **One narrowing, and it was measured before it was chosen.** A tombstone whose **successor chain leads to the row being reinstated** does not block it: it handed those words over at the retirement that named the successor, so its claim is the stale one. Without that narrowing two tombstones on one word block **each other** and neither is ever reinstatable — and `reinstate` has no `acknowledge` and no `force`, so there would be no way back at all. **A guard that closes a legal operation is the defect this row shipped once already**, at `304967a`. `C9-38`. A walk that hits `_IDENTITY_CHAIN_CAP` REFUSES rather than excuses: Rule U says *we could not finish looking* is not *there is nothing there*, and at a non-overridable identity guard the safe end of that is the refusal.

Not `alias_collision`, which §5.9b defines as *two ACTIVE entries holding one word between them* — the state here is one live row and one tombstone, so `C16-06`'s whole-store invariant does **not** fail. What fails is that the tombstone becomes **permanently un-reinstatable**, which is the governance act ruling **R11** created `reinstate` to provide. Its own value for the reason trip 14 minted `word_previously_retired` rather than reusing `name_previously_retired`: *this word is spoken for by a tombstone* and *these two extents may not be unified* are **two facts**, and one word for two facts is §2.3's Cause B.

> **The rule is about a tombstone's ALIASES and deliberately not about its NAME.** §5.8 keeps a tombstone's words by design, and standing rule (c) calls them an *unconsumed permission* — but a retired row's own **name** being written as an alias onto another row is the ordinary post-retirement succession `C12-09` **blesses** when the extents agree. Refusing that here would reverse a narrowing this register pinned on purpose. The name half is already answered at the mint doors, by `name_previously_retired` (`C10-19`, the eighth trip). **The alias half is what no door had ever asked.**

> **Ordering.** It runs **after** §5.10's refusals #1/#2/#3 and after `alias_collision`. Putting it first was this fix's own first defect, caught by `check_merge_guard.py` within one run: both facts can be true of one write, and answering an identity collapse the older guards refuse correctly with *this* reason changes what the door says about a state it already handled.
...
> **And until row 6c that sentence was half true: the guard validated a transfer this call did not perform** *(ruling **R75**, decided by a design test rather than by a patch)*. `retire(successor=)` writes the tombstone and nothing else, so *"every alias the retired row carries is re-pointed at the successor"* described a write that did not exist — the kill row's **tenth** trip in a different dress, *one door disagreeing with itself*, in a guard **R53** barred row 6b's lineage from re-comparing without a design test.
>
> **The design test ran on all three legs, over the two alias shapes, and each half of the disagreement is true of one shape.** An alias that **also has a row** — what `merge_types` leaves behind, since the absorbed name becomes a retired row whose `successor` names the survivor — survives the retirement at `existing / <successor> / 1.0`, and what carries it is the **succession chain**: the `aliases` field plays no part, and the successor's alias list is still empty afterwards. An alias with **no row of its own** (what `import_types` writes), or one naming a retired row that does not point back at the holder — the fourth trip's own shape, and the case this guard actually fires on — went from `existing / commentable / 1.0` to **`proposal / None / 0.3568`** and **`0.5556`**, on sqlite and on Postgres, for entities and for predicates. `_alias_map` scans **active** rows only, so the moment the holder is retired the word it answered for is held by nothing.
>
> **So the missing write was the defect and the guard's reading was the intended one.** `retire(successor=)` now transfers `rec.aliases` onto the successor, deduplicated, **in the same transaction as the tombstone** and **only where the identity guard passed on exactly those words** — *the write a guard permits and the write a call performs must be the same write.* The retired row keeps its own aliases, as `merge_types` leaves them on the absorbed row: a tombstone is a record of what a word meant, and editing it would be rewriting a provenance-bearing row (§5.8). The retired row's **own name** is deliberately **not** added — the succession chain already answers it at 1.0, and a second home for one fact is `EDGES.md` §2.4's rule. `C9-26`, `C9-27`, `C9-28`; [`check_merge_guard.py`](../tools/check_merge_guard.py) gains an **eighth axis** driving both alias shapes through this door on every leg, proved by mutation.
>
> **Row 6c's THIRD adversarial round closed the three things R75's write left open on this field, and all three are about a word MOVING with nobody told.** *(1)* The transfer lands on the **successor** and this call returns the **tombstone**, so the act was invisible at the call performing it: the returned entry now carries **`aliases_transferred:<successor>`** (§5.4), because reading the survivor's event log means already knowing to look. *(2)* The filter that drops the successor's own name was **one-sided** — the **retired row's own name** still transferred, which is exactly the write the paragraph above forbids, arriving by a third route; both are dropped now, and the words skipped are stated in the event's `aliases_not_added` rather than silently filtered. *(3)* `import_types` writes `aliases` **wholesale**, so an ordinary import **erased** a transferred word with no refusal, no warning and no event, leaving the transfer's own event asserting a fact the store no longer held — it now warns **`aliases_removed:<word>`** and files an `aliases_removed` **event**, which is §5.8's *a correction is a new event* applied to the removal half. `C9-32`, `C9-33`, `C9-34`, `C12-19`.
...
**`alias_collision` — the twenty-first, added in the same change as the guard.** Returned by `reinstate` (§5.9b) — **and, from row 6c, by `merge_types` (§5.10)** — when a write would leave two **active** entries with one word between them — the name held as an alias by a live type, or the reinstated type holding a live name as one of its own. Not `successor_active` (that is about the retirement's declared successor, and this collision is reached precisely by retiring that successor first), not `cross_namespace_merge` (this is inside **one** namespace, which is the case §2.6 exists to prevent rather than preserve), not `retired_operand` (nothing here is an operand of a merge). It says the one thing none of the twenty said: *the word you want back is already spoken for by something alive.* Added by row 3e's first adversarial round, which reproduced the four-call walk into mechanism 4.

> **`merge_types` is the FIFTH door to be given this refusal, and it is the kill row's THIRTEENTH trip** *(row 6c, round 2)*. The seventh trip closed `C16-06`'s invariant at four write doors — `propose_type`, `approve`, `reinstate` and `import_types` — and **`merge_types` asked at none of them**, because §5.10's identity guards compare the two **operands** and never ask whether the words being **moved** are already held somewhere else. **[Observed, five ordinary calls, no alias and no import]** merging one tombstone into two different survivors, one round of ordinary vocabulary growth apart, left both survivors answering to the absorbed word, with `resolve_type` answering one of them at **1.0** on a pair `merge_types` itself refuses `predicate_merge` non-overridably. **Non-overridable, and `retired_operand` does not buy past it**: that acknowledgement is about a row's *status*, which is evidence, and this is *identity* — `C9-20`'s own distinction. A scan that could not finish **warns** rather than refusing, which is `C12-13`'s and `C4-12`'s lesson. `C10-20`.
