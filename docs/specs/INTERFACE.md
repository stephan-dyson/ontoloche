...
A project whose thesis is that governed vocabularies resist rot does not ship an open-ended `reason` string in its own contract. `Refusal.reason` takes exactly these **thirty-two** values — eleven defined in this document, three introduced by [`PACKAGE.md`](PACKAGE.md) v0 and adopted here, one added by ruling **R4**, four by [`EDGES.md`](EDGES.md) v0, two by ruling **R11**, seven by [`ACTIONS.md`](ACTIONS.md) v0, one by row 4d's first adversarial round, one by its third, **one by ruling R73 in row 6c**, and **one by ruling R91 in row 6d** — the transfer half of the fifteenth and sixteenth kill-row trips:
