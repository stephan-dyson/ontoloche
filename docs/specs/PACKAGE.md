...
**C9 — `retire` and `reinstate` (18).** Mechanism **3**.
...
**C17 — edges: the store and the read seam (40).** `EDGES.md` v0, row 4b; **C17-35 … C17-40** by row 4c (ruling **R34**). Mechanisms **C** (dominant, and [Observed] rather than assumed) and **4**.
...
| C17-33 | **a merge makes the walk incomplete and the report says so** (`EDGES.md` rule `4.3-14`). `merge_types` is the sanctioned answer to mechanism **4** and rewrites no edge, so a caller who resolved to the canonical type before walking — the CORRECT thing — got `known=0`, **`complete=True`** and an empty `warnings` about edges sitting in the store under the absorbed name. The walk still does not follow the chain (D-4b-15, **Q33**); it stops claiming otherwise | **4** |
