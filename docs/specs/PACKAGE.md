...
**C3 — `resolve_type` (29).** Mechanisms **2**, and **1** as the gate. *(19 → 27: **eight added by row 6f**, founder ruling **R99** — four by the build (`C3-20`…`C3-23`) and **four by its first adversarial round**, every one of which closes a mutation that had passed the entire suite.)*

| id | asserts | note |
|---|---|---|
| C3-01 | `existing` sets `confidence` to a float — **AMENDED by ruling R99, row 6f, 2026-09-09: `confidence` may be `None` on an `existing` REDIRECT whose identity claim could not be scored** (Rule U; `INTERFACE.md` rule 5.3.2-9). The float still holds for every non-redirect `existing` | |
...
| C3-14 | **an identity claim is re-verified where it is MADE — the Q56 default (row 4d), and its EXPENSIVE half (row 6f)** (§5.3). A retired predicate redirected to its live successor still answers `existing`, and when the two extents that claim stands on no longer agree the returned entry carries **`identity_stale`**; when they still agree it carries nothing. ~~and still answers at confidence **1.0**~~ — **SUPERSEDED by founder ruling R99, 2026-09-09 (`read`): the confidence is now LOWERED to how much of the claim still holds**, and this id asserts that rather than the 1.0 it used to. Struck rather than rewritten because 1.0 is what the registry promised from 2026-08-30 to 2026-09-09, exactly as `INTERFACE.md` rule 5.3.2-3 is struck. Both halves, on all three legs. *(The kill row's sixth trip: every identity guard compares extents at WRITE time and this call grants 1.0 at READ time. Rule U's fourth operand — STALE is not equal. The negative is the half a careless fix breaks: a warning that never turns off is noise, which is row 4c's own `predicate_requires_review` lesson.)* | **kill row** |
...
| C3-17 | **finding X1** *(row 6d, round 1; ruling **R90**)*. `_search_namespaces` is the only guard in this package that reads more than one namespace, and it was built out of **none** of the identity machinery: it tested the retired row's **name only**, so a word a live tombstone in another namespace still answers to **as an alias** was invisible — under a **`complete=True` seal**, which is Rule U's confident negative in the call ruling **R6** exists to prevent | R6 / rule (c) |
| C3-18 | **finding X2** *(row 6d; **R90**)*. The cross-namespace read compared **bytes** while `_word_rows`, `_alias_holder` and `_alias_clash` have compared by `identity_key` since the SEVENTH trip. `NAME_RE` admits `foo__bar`, so `dpr:bike__lane` against a `dot` caller's `bike_lane` is UC3's ordinary case rather than a contrivance | R6 / trip 7 |
| C3-19 | **finding X7** *(row 6d, round 2)*. Every `alternatives` label must name a **ROW**. Until this row keyed the cross-namespace probe, `exact_elsewhere` fired only on a **byte-exact** match — so the candidate WAS the row's name and `f"{other}:{candidate}"` named a real row. **Keying the probe widened the producer and left this consumer alone**, so the list carried `('dpr:bike_lane', None)` for a store holding only `dpr:bike__lane`, **under a `complete=True` seal**, reproduced on live NYC data. It broke three rules at once: §5.3.1 rule 3 (the `<name>` named nothing), rule 5 (`None` means *nothing scored it*, while that row was scored **1.0** two entries above), and rule 5's note that a taken word is listed once — `known` counted **3 for 2 rows**. **A confident POSITIVE about a row nobody holds, in the call R6 added to end confident NEGATIVES** | R6 / Rule K |

| C3-20 | **the stale confidence is DERIVED, not chosen** (§5.3 rule 5.3.2-9, row 6f, founder ruling **R99**). `min(resolver_score, Jaccard(L, R))` over the two written extents the call already reads. Driven on **two stores whose Jaccard differs**, each asserted against the value the test computes from the extents it reads back itself — so **no constant passes both**, and a containment ratio fails the first. *(Row 4d's third round deleted two rule rows with the whole suite green; this id exists so that cannot happen to this one.)* | R99 |
| C3-21 | **an action family's identity is verified by its DECLARATION** (§5.3 rule 5.3.2-10, row 6f). An action family has no extent — §5.10's refusal #2 is skipped for actions by design — so the read compares declarations instead, and two families whose declarations disagree answer **below 1.0** with `identity_stale`. **This is the governance register's entry A3 at the READ**, and the control is the half a careless fix breaks: identical declarations still answer **1.0** with no warning | R99 / A3 |
| C3-22 | **a stale redirect is NEVER a refusal** (§5.3 rule 5.3.2-11, row 6f). R99 authorised refusing and this row declines to: in every reachable state the registry can name the correct answer, and refusing on an unknowable read would ban the door on a legal declared-degraded backend (`C10-09`, `C3-13`, `C12-13`). The id exists so a later row cannot quietly turn the score into a refusal without a ruling | R99 |
| C3-23 | **`min_confidence` governs the redirect path** (§5.3 rule 5.3.2-12, row 6f). **[Observed]** it did not: the exact-hit redirect returned before reaching the test, so `min_confidence=2.0` still answered `existing` at 1.0 — invisible while the answer was always 1.0, live the moment 5.3.2-9 lowers it. No bar → unchanged; at the bar → `existing`; above it → `none` with the survivor still in `alternatives`. **Also pins Rule K on both call sites** after this row reintroduced `C3-19`'s double-count on the alias branch | R99 / Rule K |
| C3-24 | **a REORDERED declaration is the SAME declaration** (§5.3 rule 5.3.2-10, row 6f, round 1). Row 6d's round 3 compared `effects` with `!=` and **closed a legal operation** for two rounds by refusing families whose governance was identical and whose effects were merely written in a different order. `ACTIONS.md` §1 says *"no ordering"*. **The protection was UNPINNED until this id** — round 1's mutation lens made the comparison order-sensitive again with the whole suite green | R99 |
| C3-25 | **an UNDECLARED action family is unknowable, not agreeing** (§5.3 rule 5.3.2-10, row 6f, round 1). `ACTIONS.md` §2.2-1 permits a family with no declaration; collapsing one into a human-approval-only family is **no evidence** the two are one thing. Round 1's mutation lens deleted the guard and the suite stayed green — an undeclared family silently became **fully vouched**. The FIRST trip's operand, at a new door | R99 / kill row |
| C3-26 | **an unscorable identity does not clear a bar** (§5.3 rule 5.3.2-12, row 6f, round 1). Joined on a capable backend and read through one declining `indexes_membership` — `PACKAGE.md` §2.6's production path — the identity cannot be scored, and a caller asking for `min_confidence` is answered `none` rather than `existing`. Round 1's mutation lens flipped the branch and the suite stayed green, because every bar case used a **scored** pair and every unscorable case used **no bar**: both halves covered, their intersection not | R99 / Rule U |
| C3-28 | **an UNKNOWABLE read answers `confidence: None`, not zero** (§5.3 rule 5.3.2-15, row 6f round 2). 5.3.2-9's formula is a Jaccard over two extents and an unknowable read produces neither, so this is the one cell where the formula has no input — and until round 2 it was the one cell with **no rule**. Drives the SAME store through a capable reader and one declining `indexes_membership` and asserts the two answers DIFFER, which is the property 5.3.2-15 states: **this confidence measures what THIS READER could establish, not the identity alone.** *(Found by a supervisor spec-read after four lenses on the implementation missed it; the id that touched the state, `C3-26`, was `pytest.skip`ping on the very value it existed to assert — the third occurrence of that shape here.)* | R99 / Rule U |
| C3-29 | **a key declared EMPTY is not a key never declared** (§5.3 rule 5.3.2-16, row 6f round 2). **A defect from `E`'s own FAMILY -- *a confident answer standing in for a fact the system had or could not have* -- in the operand this row added to remove `E`'s DELIVERY.** *(Not statement `E` itself, which is a write-time fact treated as true at read time; naming a family member after the family is how a name stops distinguishing anything.)* the first cut derived its key set from the union of the two stored dicts, so a family positively declaring `reachability=[]` and one that never declared the key both flattened to the same empty set and the pair answered a **CLEAN 1.0 with no warning**. `ACTIONS.md` §2.2: *an empty list is a positive declaration, not a forgotten field.* Pins both halves — present-vs-absent is not agreement, and absent-from-BOTH is, out of the fixed eight rather than a denominator that moves with whatever happens to be stored. *(Survived four round-1 lenses because `action_attributes()` always writes all eight keys, so no fixture in the suite could construct the absence.)* | R99 / kill row |
| C3-27 | **every reachable stale redirect scores 1.0 BEFORE the agreement is applied** (§5.3 rule 5.3.2-9's `prose-only:` half, row 6f, round 1). Chasing a surviving mutation of the `min` produced a finding rather than a test: **[Observed]** by instrumenting the composition, the resolver's score there is *always* 1.0 — `C3-11` made the redirect a **registry** guarantee precisely so it would stop depending on the shipped scorer — so `min(1.0, J)` **is** `J` and no id can distinguish the two implementations. This id pins the **precondition** instead: if a stale redirect ever arrives scoring below 1.0, the tag's reason stops being true and this test fails and says so | R99 |

**C4 — `propose_type` (16).** Mechanism **1**.
...
| C4-15 | **the kill row's FOURTEENTH trip** *(row 6c, round 3; provenance by BISECT — live at `ddf2f5e` and at `664d3a5^`, so it **predates row 6c entirely**, and R75 did not introduce it but **widened its blast radius** from one tombstone to every successor in a retirement chain)*. A word a **RETIRED** row still answers to as an **alias** was free at this door: **[Observed, five ordinary calls, no merge, no `force`, no acknowledgement]** the mint succeeded with **empty warnings**, and the store then refused **both** `merge_types` (`predicate_merge`) and `reinstate` (`alias_collision`) NON-OVERRIDABLY — the tombstone permanently un-reinstatable, which is ruling **R11**'s own governance act. The guard was at the wrong end: it blocked the row wanting its word back, not the call that took it. `word_previously_retired:<holder>`, nothing written — at `propose_type`, the first mint door | **kill row** |
| C4-16 | **the kill row's FIFTEENTH trip** *(row 6d, round 1; countersigned by ruling **R91**)*. The fourteenth trip's fix was **kind-scoped**: `_word_rows` was called `kind=`-scoped at all three mint doors, so a word a retired row still answered to as an **alias** was free to a mint of **another kind** — while `_alias_clash`, which applies no kind filter, saw it at `reinstate` and refused `alias_collision` **non-overridably**. One store, one word, two guards, opposite answers, and the tombstone left permanently un-reinstatable. **[Observed, five ordinary calls, no `force` on the mint, no acknowledgement, both directions, both legs]**, and **not** action-specific. **PREDICTED IN WRITING at `d4b86a8` before any lens ran** — `6D-RUN.md` §0.3's **T14**, by cell | **kill row** |
| C16-07 | **the narrowing beside `C4-16`, and the half a careless fix deletes.** `PACKAGE.md` §4.1 blesses one word under two kinds, and `_alias_holder` gates its NAME side on `other.kind == kind` for that reason. The fifteenth trip's fix makes a **tombstone's** words kind-blind and must leave **LIVE** rows alone: two active rows may still share a word across kinds | narrowing |

**C5 — `approve` / `reject` (15).** Mechanism **1**.
...
| C5-13 | **the kill row's FOURTEENTH trip** *(row 6c, round 3; provenance by BISECT — live at `ddf2f5e` and at `664d3a5^`, so it **predates row 6c entirely**, and R75 did not introduce it but **widened its blast radius** from one tombstone to every successor in a retirement chain)*. A word a **RETIRED** row still answers to as an **alias** was free at this door: **[Observed, five ordinary calls, no merge, no `force`, no acknowledgement]** the mint succeeded with **empty warnings**, and the store then refused **both** `merge_types` (`predicate_merge`) and `reinstate` (`alias_collision`) NON-OVERRIDABLY — the tombstone permanently un-reinstatable, which is ruling **R11**'s own governance act. The guard was at the wrong end: it blocked the row wanting its word back, not the call that took it. `word_previously_retired:<holder>`, nothing written — at `approve`, and this is the door **ruling R40 forces every `kind="predicate"` down**, so the guard was structurally unavailable for the one kind the kill row is about. The word is FREE when the proposal is made and spoken for by the time it is approved | **kill row** |
| C5-14 | **the kill row's FIFTEENTH trip at the door ruling R40 forces every `kind="predicate"` down** *(row 6d, round 1; **R91**)*. `approve` writes the row days after `propose_type` judged it, so `_write_approved`'s tombstone scan is the one that decides — and it was `kind=`-scoped like its two siblings | **kill row** |
| C5-15 | **the kill row's TWENTIETH trip** *(row 6d, round 2; countersigned by ruling **R93**)*. The EIGHTH trip minted `name_previously_retired` and applied it at `propose_type` and `import_types` — and **never at `_write_approved`**, the door ruling **R40** forces every `kind="predicate"` down. A tombstone whose own NAME is another spelling of the approved word was minted over with **`warnings=()`**, and `reinstate` was then refused `alias_collision` **non-overridably**. Four ordinary calls, no `force`. **[Observed, bisect]** pre-existing at `d4b86a8`: change 1 widened `_word_rows` past its kind filter and left the filter that DISCARDS the scan's result alone — *a widened matcher is a minted rule and its consumers are its doors* (R93) | **kill row** |
...
**C9 — `retire` and `reinstate` (38).** Mechanism **3**.
...
| C9-26 | **§5.9, ruling R75, row 6c** — `retire(successor=)` now WRITES the alias transfer its guard has been checking since row 4d. **[Observed, the design test, sqlite and postgres, entity and predicate]** an alias of the retired row went from `existing / commentable / 1.0` — the confidence §5.3 calls a guarantee — to `proposal / None / 0.3568`, because `_alias_map` scans ACTIVE rows only and the holder had just been retired. The successor takes the words, deduplicated; the tombstone keeps its own | **4** |
| C9-27 | **§5.9, ruling R75** — the half a careless fix deletes: a retirement the identity guard REFUSES writes nothing, neither tombstone nor alias. *The write a guard permits and the write a call performs must be the same write* — the kill row's TENTH trip stated from the other end. Fixture is Q77's own STALE shape: the alias is legal when written, the world moves, and the retirement arrives with the row pair agreeing and the alias pair not | **4** |
| C9-28 | **§5.9, ruling R75** — the boundary. The retired row's OWN NAME is deliberately not added as an alias: the succession chain already answers it at 1.0, so adding it would be a second home for one fact (EDGES.md §2.4) and a write no guard on this call examined. An id rather than a comment, because *"we deliberately did not write that"* is what a later row silently reverses | **4** |
| C9-29 | **The kill row's TWELFTH trip** *(row 6c, round 1; ruling R75's write, and the lens proved the provenance by BISECT against `664d3a5^`)*. `retire` read `.status` twice and **both times on the successor**, so a repeat retirement toward a second successor copied the retired row's aliases onto a **second live row while the first still held them** — two active rows on one word, `C16-06` and mechanism 4, on a pair `merge_types` refuses non-overridably. **The guard was evaluated once for a call that can run twice.** Fixed as `reinstate` answers the identical question: `retire_no_op:already_retired`, no write | **4** |
| C9-30 | *(row 6c, round 1)* the **successor's own name** is not transferred into its own aliases — `_alias_identity_breach` skips the self row when it compares, so this passed the guard: the *second home for one fact* `C9-28` avoids for `rec.name`, arriving by the other route | **4** |
| C9-31 | *(row 6c, round 1)* the alias transfer is its **own event** (`aliases_transferred`) and not a `retired` event on the live survivor. It corrupted nothing — `_lifecycle_collisions` takes `detail["successor"]` and this has none — and **safe by the accident of a missing dict key is not safe**: one event value carrying two facts is INTERFACE.md §2.3's Cause B | **4** |
| C9-32 | *(row 6c, round 3, closing round 2's fix-auditor MAJOR)* **`C9-30`'s filter was ONE-SIDED.** It dropped the successor's own name and nothing else, so a retired row carrying **its own name** in its own alias list still transferred it — **[Observed]** `taggable.aliases == ('commentable', 'zeta')` after `retire("commentable", successor="taggable")`, which is exactly the write `C9-28` forbids, arriving by a third route. Both are dropped now, and the skipped words are **stated** in the `aliases_transferred` event's `aliases_not_added` rather than silently filtered: a caller who hands this call two words and sees one arrive is owed the reason | **4** |
| C9-33 | *(row 6c, round 3, closing round 1's integrator MINOR)* the transfer is announced **at the call**: `retire(successor=)` returns the tombstone with **`aliases_transferred:<successor>`**. R75's write lands on the SUCCESSOR and this call returns the TOMBSTONE, so the one act that moved a word between identities was invisible to the caller performing it — the event is on the survivor, which is the right home for the history and the wrong one for the caller. The thirty-fifth warning value, minted with its INTERFACE.md §5.4 row per **R3** | **4** |
| C9-34 | *(row 6c, round 3, closing round 1's kill-row MINOR)* the `no_consumer_evidence` sentence is about **this row's kind**. **[Observed, `DegradedAdapter(indexes_membership=False)`]** retiring two ordinary ENTITIES was refused with *"this backend cannot compute a predicate's extent"* — about a row that has none. The refusal is correct and stays; `C9-19`'s own class one noun along, *the right refusal reached by a sentence about the wrong fact*, in the field a caller reads to decide what to do next | — |
| C9-35 | *(row 6c, round 3's own fix-auditor lens, MINOR — a defect in this round's own fix, one commit old)* the skipped words are stated **even when every word was skipped**. `C9-32`'s claim and INTERFACE.md §5.9's sentence — *the words skipped are stated rather than silently filtered* — were both true only when at least one word ALSO survived, because the whole block was gated on `repoint_words`. **[Observed]** a tombstone whose aliases are exactly the successor's name, or exactly its own, transferred nothing, warned nothing and filed nothing: the pure case, where the statement is the entire value. The `aliases_transferred` **warning** stays gated on an actual write | **4** |
| C9-36 | **the kill row's SIXTEENTH trip at the TRANSFER door** *(row 6d, round 1; **R91**)*. Ruling **R75** re-points a retired row's aliases onto its successor, and nothing asked whether a **second** tombstone still answers to one of those words. `word_held_by_tombstone`, **non-overridable** — `force` overrides what could be SEEN, never what would become TRUE | **kill row** |
| C9-37 | **the kill row's TWENTY-THIRD trip** *(row 6d, round 3; ruling **R94**)*. `reinstate` is the **fourth** door that makes a word answer at **1.0**, and the one door `word_held_by_tombstone` was never wired to: `9a4e140` minted the value and applied it at `retire(successor=)`, `merge_types` and `import_types` — **three doors where the surface has four**. Standing rule (d) by number, at a rule this row minted itself. **[Observed]** in four ordinary calls, no `force`: `reinstate` returned a `TypeEntry` with `warnings=()`, `resolve_type` answered the word at 1.0, and the other tombstone was then refused `alias_collision` non-overridably — **permanently un-reinstatable**, which is ruling R11's own governance act | **kill row** |
| C9-38 | **the narrowing `C9-37` must not be written without, and it was MEASURED before it was chosen** *(row 6d, round 3)*. The naive consistent form **deadlocks**: two tombstones on one word block **each other**, and `reinstate` has no `acknowledge` and no `force`, so neither ever comes back. §5.9's successor mechanism discriminates — a tombstone whose chain leads to the row being reinstated **handed those words over** at the retirement that named the successor, so its claim is the stale one and the receiver must still be able to reclaim them. **A guard that closes a legal operation is the defect this row shipped once already**, at `304967a` | narrowing |

**C10 — `merge_types`, and the doors its operands come through (27).** Mechanism **4**.
...
| C10-20 | **The kill row's THIRTEENTH trip** *(row 6c, round 2)*. `merge_types` is the FIFTH write door and it asked `C16-06`'s question at none of them: `_identity_breach(left, right)` compares the two OPERANDS and never asks whether the words being MOVED are already held elsewhere. **[Observed, five ordinary calls, no alias and no import]** two merges of one tombstone into two different survivors left **two ACTIVE rows answering to one word**, `resolve_type` at **1.0**, on a pair `merge_types` refuses `predicate_merge` non-overridably. Trip 12's class at the sibling caller: a tombstone's words are an **unconsumed permission**, so the obligation belongs to the ROW's words rather than to the caller. `alias_collision`, non-overridable | **4** |
| C10-21 | **the kill row's SIXTEENTH trip at the FIFTH write door** *(row 6d, round 1; **R91**)*. `merge_types` writes `(left.name, *left.aliases)` onto `right`; `_alias_clash` asks only whether an **ACTIVE** row answers to one of them, by design. Both operands are excluded from the tombstone scan, and leaving them in was the fix's own near-miss — `left` becomes a tombstone in this very write, so a scan that did not skip it would make the door refuse **itself**: the TENTH trip's sentence pointed at the fix before it shipped | **kill row** |
| C10-22 | **the kill row's EIGHTEENTH trip** *(row 6d, round 1; countersigned by ruling **R92**)*. `merge_types` bound `_alias_clash`'s `why` and **never read it** — `alias_check_incomplete` appeared **zero** times in the method, while the comment above the call said *"it warns and proceeds"* and it did neither. So a merge over a scan the backend cut short was **byte-identical to the caller** to one over a scan that finished, and the short one proceeded to two live holders and a confidence-1.0 answer. The FIFTH trip's operand un-applied to a guard the THIRTEENTH trip's fix added: `_alias_clash` did not exist when trip 5 closed *partial is not equal*, so it inherited the rule's **exemption** rather than the rule. Rule U on the LOOK — reported, never refused on. **The detail NAMES the scan**, because this door now runs two that can each fail to finish | **kill row** |
| C10-23 | **the kill row's SEVENTEENTH trip — ruling R80's Q82, constructed** *(row 6d, round 1; **R92**)*. The escape that excuses a holder which is another spelling of `left.name` was evaluated over **one** holder, and `_alias_clash` returned whichever row the backend paged first while `PACKAGE.md` §3.3 guarantees **no ordering at all**. **[Observed]** over all 120 page orders of one five-row store: **60 refused, 60 swallowed**, the swallowed half leaving two ACTIVE rows on one word with `resolve_type` at 1.0 — on a pair the same call refuses `predicate_merge` non-overridably. The escape is now evaluated over the whole SET. `OrderedAdapter` is a **legal** backend, not a broken one | **kill row** |
| C10-24 | **the NINETEENTH trip at the door that asks refusal #1 DIRECTLY** *(row 6d, round 1; **R92**)*. `import_types` reaches #1 through the alias door; `merge_types` reaches it through `_identity_breach` itself, where nobody passes computed gate sets, so both sides are read off stored rows and both come back blank on a backend declining `indexes_membership`. Driving only the door the trip was found at is *a rule minted at the caller that prompted it* — **standing rule (d)** | **kill row** |
| C10-25 | **the kill row's TWENTY-SECOND trip — the escape excuses a STRANGER** *(row 6d, round 2; **R93**)*. Change 2 fixed *which* holders the escape is evaluated over and never asked *what excuses one*. The clause read `not same_word(holder, left.name)` — keyed by **word** — on the ground that a holder spelled differently is `left` itself under another skin. **§4.1 blesses one word under two kinds**, so it need not be: `alpha_`(entity) beside `alpha`(predicate) is **two rows**, and excusing the entity let `beta` take the word while the entity kept it — both answering at **1.0**, on a pair the alias door refuses `kind_mismatch` **non-overridably** in the same store. The escape now excuses exactly `left`, same name and same kind, and **exact rather than keyed on purpose**: a row whose name is a variant spelling of `left.name` is a DIFFERENT ROW. Where `indexes_membership` is False the extents are unknowable and `predicate_merge` refuses first, which the id records as NOT REACHABLE rather than as a pass | **kill row** |
| C10-26 | **findings G6 and A15 — the verdict was made order-independent and the DETAIL was left in scan order** *(row 6d, round 2; **R93**)*. `holder`, `holders` and `held_by` took different values across page orders of ONE store, at the door whose own comment asserts *"Every holder, so the detail cannot depend on page order either"*. **A guard that answers the same and SAYS something different is the EIGHTEENTH trip one field along**, and the axis built for page order compared verdicts and never a detail. `_alias_clash` now returns its holders **sorted**, and axis 12 compares the detail | **kill row** |
| C10-27 | **the accumulator dropped on every refusal path — findings G4 / A14 / K5 / X11, reached by FOUR lenses independently** *(row 6d, round 2; **R93**)*. Every door accumulates skip notes as it walks its guards and every one of them discarded the accumulator the moment it refused, because `Refusal` had **no `warnings` field at all**. So the NINETEENTH trip's own value — `identity_guard_skipped`, minted precisely so a caller is told when a guard could not run — **reached a caller only when the call succeeded**. That is **standing rule (d)** at a rule this row minted itself, and it is the worse half: a caller told *no* cannot tell *we checked* from *we could not check*, which is Rule U at the surface rather than inside a guard. This id is `C10-24`'s deliberate mirror — the same store, the same skipped guard, a refusal arranged to fire after the note was collected | **kill row** |
...
**C12 — Foundry import mapping, and the fourth door into the kill row (29).** From 0.3 consequence 2 / `INTERFACE.md` §2.5.
...
| C12-19 | *(row 6c, round 3, closing round 1's kill-row `m1` and round 2's kill-row `m4`)* an import that **REPLACES** a standing row's aliases says which words it took away: `aliases_removed:<word>` on the returned entry, and an `aliases_removed` **event**. **[Observed]** a `retire(successor=)` transferred `zeta` onto `beta` and filed `aliases_transferred`; an ordinary import then left `beta.aliases == ()` with no warning at all — and the transfer's own event **still standing, asserting a fact the store no longer held**. A word that resolved at 1.0 stopped resolving, and §5.3 calls that a guarantee. Not a refusal: an import is a vocabulary arriving *already decided*. INTERFACE.md §5.8's *a correction is a new event*, applied to the removal half | **4** |
| C12-20 | *(row 6c, round 3's own fix-auditor lens, MAJOR — a defect in `C12-19`, one commit old)* the alias diff runs against the row `put_type` will actually **overwrite**. `standing` falls back to the EIGHTH trip's variant-spelling row, which is right for the `name_previously_retired` and `live_consumers` guards — they ask *is this WORD spoken for?* — and wrong for a diff, which asks *what is this ROW losing?* With an incoming `deprecated` row the `name_previously_retired` branch is skipped, so **[Observed]** `beta_` retired holding `zzz_word`, an import of a `deprecated` `beta` answered `aliases_removed:zzz_word` and filed the event on `beta` — while `beta_` **still held the word** and `beta` never had. A **fabricated correction** in the audit trail §5.8 exists to keep truthful, committed by the id minted to remove that class. `_word_rows`' own docstring names `borough_` as a spelling two agencies produce, so UC3 reaches it ordinarily | **4** |
| C12-21 | **the kill row's FOURTEENTH trip** *(row 6c, round 3; provenance by BISECT — live at `ddf2f5e` and at `664d3a5^`, so it **predates row 6c entirely**, and R75 did not introduce it but **widened its blast radius** from one tombstone to every successor in a retirement chain)*. A word a **RETIRED** row still answers to as an **alias** was free at this door: **[Observed, five ordinary calls, no merge, no `force`, no acknowledgement]** the mint succeeded with **empty warnings**, and the store then refused **both** `merge_types` (`predicate_merge`) and `reinstate` (`alias_collision`) NON-OVERRIDABLY — the tombstone permanently un-reinstatable, which is ruling **R11**'s own governance act. The guard was at the wrong end: it blocked the row wanting its word back, not the call that took it. `word_previously_retired:<holder>`, nothing written — at `import_types`, the third mint door. Shipping this at one door of three is the single sentence of the ninth, tenth and eleventh trips | **kill row** |
| C12-22 | **the kill row's FIFTEENTH trip at the third mint door** *(row 6d, round 1; **R91**)*. `import_types` is a NAME door in its own right; shipping the kind-blind scan at `propose_type` alone would be *a fix applied at one call site of three* — the single sentence of the ninth, tenth and eleventh trips | **kill row** |
| C12-23 | **the kill row's SIXTEENTH trip** *(row 6d, round 1; countersigned by ruling **R91**)*. The trip-14 rule bound the incoming **NAME** only. A word a tombstone still answers to, arriving in the incoming row's **`aliases`**, was written with **no refusal and no warning**; `resolve_type` then answered the new row at 1.0 and `reinstate` refused `alias_collision` non-overridably **with `path_back=None`** — worse than the door it descends from. `_alias_identity_breach`'s keyed scan defaults `match_aliases=False`. **PREDICTED BY LINE at `d4b86a8`** — `6D-RUN.md` §0.3's **T11** named `registry.py:7023` before any lens existed | **kill row** |
| C12-24 | **the kill row's NINETEENTH trip** *(row 6d, round 1; countersigned by ruling **R92**)*. §5.10's refusal **#1** compares CONSUMER SETS, and on `indexes_membership=False` a stored row's `predicates` come back empty, so both sides read blank and #1 compares equal **vacuously** — and the door said **nothing at all**. The same five ordinary calls: refused `different_consumer_sets` **non-overridably** on sqlite, written with `warnings=()` on the degraded adapter, `resolve_type` at **1.0**. UC1 Tenshen's own declared shape, and the FIRST trip's backend. Trips 1 and 9 asked whether *unknowable* equals *equal* or *different*; this asks whether it equals **nothing to say**. The comparison is deliberately unchanged (R53's boundary; refusing would ban the door on that backend); the caller is now **told** — `identity_guard_skipped`, the thirty-eighth warning value. **Entities, not predicates**, because a predicate pair is refused by #2 for an unrelated reason | **kill row** |
| C12-25 | **finding X6** *(row 6d; **R90**)*. `import_types` takes ONE `namespace` per batch, and a row carrying its own had that key **silently dropped** — the identity written into the CALLER's scope with `warnings=()`. **Not a refusal**: the write is legal and the mapping is the documented one; what was missing is that it says so. Accepted-and-ignored is the `mark_reviewed` shape row 6c fixed one call along | said out loud |
| C12-26 | **the kill row's TWENTY-FIRST trip** *(row 6d, round 2; **R93**)*. `r.name != name` discarded the **byte-identical** tombstone; `standing` is `None` exactly when no row *of that kind* carries the name, so such a row can only be a tombstone of **another kind** — the cell change 1 dropped `kind=` to expose. **The clause was inert while the scan was kind-scoped and became live the moment it was widened.** Worse than the fifteenth trip's outcome: the tombstone became **unreachable**, because `reinstate` takes no `kind` — that residual is declined and carried as **Q96** | **kill row** |
| C12-27 | **finding X11 — the accumulator family at the door that returns no `Refusal`** *(row 6d, round 2)*. `_refused_import` returned `import_refused:<reason>` **alone**, so every note the call had collected on the way to declining the row — X6's `import_field_ignored` and the `alias_check_incomplete` values beside it — was dropped by the `continue` that followed. Filled at **five** sites, read at exactly **one**. `C10-27`'s sentence at a different shape, which is why they land in one change: **standing rule (d)**, the decline paths a success-path rule also binds — and it counts most here, since a caller told the row was refused has MORE need of *your namespace column was ignored*, not less | **X11** |
| C12-28 | **Rule U at the CALL SITE, not at the comparison** *(row 6d, round 3; ruling **R94**)*. `import_types` runs several scans that can come up short and two of them **bound the `why` and never read it**. On a legal §3.4 backend that cannot finish a page the list came back SHORT and the guard concluded *nothing found* — the guard was skipped and the row was **written**. Each report now **names its scan**, because a truncated NAME scan means a tombstone may have been missed (the twentieth and twenty-first trips) and a truncated ALIAS scan means a live holder may have been; two facts, and one value for two facts is §2.3's Cause B | **A3** |
| C12-29 | **finding A3 at the site R94 named** *(row 6d, round 3)*. The action-declaration loop read `named, _named_why` — bound, never used — so a truncating backend made `_action_declarations_diverge` see fewer rows, find no divergence, and **write the alias**: `resolve_type` at 1.0 one way while `preflight` refused the other, which is A3's own harm with A3's fix live. **Change A's consumer table did not list this site**; its *kind* claim holds, and it is incomplete on the other axis — the widening made the scan LARGER, which makes an incomplete page MORE likely, at the one call site that read incompleteness as agreement | **A3** |
...
| C16-08 | **`Refusal.warnings` is `INTERFACE.md` §5.4's closed vocabulary, enforced in `__post_init__`** *(row 6d, round 2)*. The field was added so a caller told *no* learns what the call could not check; a field that accepts anything eventually carries anything, and `TypeEntry.warnings` is validated at **exactly one site in the whole package** (`record_invocation`, marked `pragma: no cover`) — which is how `created_by` got through unchecked in row 3e, the defect `C16-05` exists for. **Written because removing the check was a mutation SURVIVOR**: the enforcement went in with the field and nothing pinned it, found by running the mutation rather than by reading the diff |
...
| C19-82 | **§2.5-13, row 6c round 1, BLOCKING** — a supplied `InputRef` whose identity segments carry `:` or `#` is refused at **both** invocation doors. `NAME_RE` binds `propose_type`'s name, binds no namespace, and bound no supplied ref at all: a `TypeRef` named `person#p-1` was stored and read back by `parse_ref` as an **`InstanceRef` naming an object that never existed**, no exception anywhere. Not the permissive reading R72 was written against — a **confident reading of the wrong thing** | **C** |
| C19-83 | **§2.5-12, row 6c round 1** — an input-determined effect (`namespace=None`) is judged for `edge_family_retired` over the namespaces the invocation's **own inputs** carry. The declaration door's reading was a false answer in **both** directions: silent where the edge landed on a withdrawn family, and warning about a family live where the edge went | **3** |
| C19-84 | **§6-12, row 6c round 1** — `invocations(unreviewed=True)` reads the **mode of record**, never the family's mode today. An ordinary amendment moved three never-reviewed invocations out of the queue and four historical `auto` ones into it, with no event and no warning. Rule 3-1's argument one call along | **1** |
| C19-85 | **§6-13, row 6c round 1** — `review_invocation` honours its `namespace` (a queue operator scoped to one tenant was draining another's) and refuses an empty `reviewed_by` with a `ValueError`, as `retire` answers an empty `reason`. `Invocation.reviewed_by` makes *who cleared this* a field rather than a walk through history | **4** |
| C19-86 | **§10-11 and §10-12, row 6c round 1** — `ProjectionReport` names the scope it answered over, and an all-zero answer says **which** empty it is. A typo'd namespace produced zeroes with `fits` naming every group and `over_by=0` — the call affirmatively answering *everything fits* about a scope holding nothing, which is rule 10-9's own sentence pointing at itself. A negative `budget` raises | **C** |
| C19-87 | **§2.5-12, row 6c round 3, closing round 2's fix-auditor MAJOR** — `C19-83`'s fix **reintroduced the false positive it was written to remove**. `_input_namespaces` unions every namespace **any** input mentions, so an unrelated input drags one in: **[Observed]** `person_links` ACTIVE in `beacon` where the edge lands, retired in `tenant_a` named only by an unrelated `cfg` input → `edge_family_retired:person_links` at **both** doors. `record_invocation` now judges an input-determined effect in the namespace the **observed** effect resolved to; `preflight`, where the landing namespace is unknowable, warns only when every candidate that HAS the family has retired it. Both of `C19-83`'s true directions still fire | **3** |
| C19-88 | **§10-12, row 6c round 3, closing round 2's fix-auditor MAJOR** — `C19-86`'s scope sentence **asserted a falsehood**. `projection` probed the scope with `list_types("action", namespace=…)`, which defaults to `include_retired=False`, so an all-retired scope was told *"holds no `kind="action"` row of any status"* — byte-identical to the typo'd-scope sentence — and the `scope_active == 0` branch was **unreachable on all three legs**. The status census is now a separate read, `include_retired=True` and **paged to exhaustion**, with the selectable pool still active-only; a scan that could not finish says *we could not read the scope*, never *the scope is empty*. **[Observed]** the enumeration's third cause cannot exist: no code path persists a `TypeRecord` with `status="proposed"` | **C** |
| C19-89 | **§6-12, row 6c round 3, closing round 2's fix-auditor MINOR** — `C19-84`'s Rule-U fallback is **stated**. On a backend keeping no `declared_policy` the defect that ruling closed is fully live for every such row, and `why_incomplete` said nothing: the comment's claim that *"the report says the set is a floor either way"* was true only of `complete=False`, which is a sentence about the FILTER. The clause names **how many** rows were judged against the family's mode today, because *some* and *all* are different facts to an operator draining a queue | **1** |
| C19-90 | **§2.5-13, row 6c round 3, closing round 2's fix-auditor MAJOR** — `C19-82`'s refusal says which of its two failures this is. A segment carrying `#` makes `ref_key` write a string `parse_ref` reads back as a **different reference**; a **namespace** carrying `:` makes it write one `parse_ref` **raises** on. One `why` for both sent the second caller looking for a misreading that does not happen. *(Where the constraint should BIND is **Q81**, not this layer's to answer)* | **C** |
| C19-91 | **§2.5-12, row 6c round 3's own fix-auditor lens, MAJOR** — a defect in `C19-87`, one commit old. **A LANDING is a fact and a candidate is a maybe**, and applying Rule U's all-retired quantifier to both **deleted a true warning**. **[Observed, sqlite and the async mirror]** one invocation landing `person_links` in `tenant_a` (retired) *and* in `default` (active) answered `()` where the `tenant_a`-only landing warns — `C19-83`'s FIRST direction restored by the fix for its second, and the answer to *can a host suppress this?*: one appended `add_edge` in a live namespace did it. An edge that reached a withdrawn family is warned about however many live families it also reached | **3** |
| C19-92 | **§10-11, row 6c round 3 — found independently by BOTH remaining lenses** (the integrator graded it BLOCKING, the fix-auditor MAJOR). `projection` built its POOL from a single un-paged `list_types` while `_scope_census` paged the same scope to exhaustion **in the same call**. **[Observed, `DegradedAdapter(page_cap=3, page_cursor=True)`, 9 families]** `counts={'ingest': 3, 'publish': 0, 'report': 0}` against a truth of `6/2/3`, `over_by=0` — *everything fits* about a scope §10 exists to say does not — and `why_incomplete` **positively asserting** *"groups no family in this scope carries: ['publish','report']"* about two groups five families carry. And a truncated pool **refused a live group as a typo**: `Refusal(action_family_unknown)` for a group an active registered family in that scope declares. One read now; the typo refusal and the unknown-groups claim are gated on the census finishing; the scope sentence is about the READ. Also the **fourth** empty cause rule 10-11 did not enumerate: a scope of active rows that DECLARE no family said nothing at all | **C** |
| C19-93 | **§6-12, row 6c round 3 — found independently by both remaining lenses**, MINOR. `C19-89`'s clause divided by the POST-filter row count while counting the PRE-filter loop. **[Observed]** `5 of 0 row(s)` — an arithmetic impossibility attached to an empty queue, in the clause whose whole justification is that *some* and *all* are different facts to an operator | **1** |
| C19-94 | **§2.5-13, row 6c round 3's own fix-auditor lens, MINOR** — a defect in `C19-90`, one commit old. It hard-coded `field == "namespace" and separator == ":"` and was right about **one of three identity segments**: a `:` anywhere before the `#` yields four segments, so `parse_ref` **raises** for `name`, for `kind` and for an `EdgeRef.family` too. The consequence is now **asked** of `parse_ref` rather than classified — a case analysis over `(field, separator)` is a second home for the grammar, and it is what went stale within one commit | **C** |
| C19-95 | **§10-5, row 6c round 3's integrator lens, MINOR.** One `_consumer_report` per evicted **family**, not one per predicate per family — **[Observed]** five `find_consumers` where two suffice, answer identical, on the call §10.1 measures at 222 families. And the half that IS an answer: an evicted family `get_type` cannot return contributed **nothing and said nothing**, so *no consumer is at risk* and *we could not look* were the same tuple; the count and the names are stated now | **C** |
| C19-96 | **§6.5, row 6c round 3's integrator lens, MINOR.** `review_invocation` accepts a review of an `auto`-mode invocation that was never in any queue — and it should: refusing to record a review a person performed is the answer §2.5 calls the worst available. What was wrong is that the event was **indistinguishable from a genuine drain**. The event carries the **mode of record** and whether the row was queued, in two keys rather than one word carrying both | **1** |
| C19-97 | **finding A3** *(row 6d, round 1; ruling **R91**)*. §5.10's refusal **#2** is skipped for `kind="action"` **by design** (`ACTIONS.md` §2.1: actions must be mergeable) and **nothing was put in its place for what an action family IS**. Two verbs with contradictory governance collapsed with no refusal: `resolve_type` answered the dead word with the survivor at **1.0** while `preflight` answered it with the **tombstone's** policy, and a Haiku-tier actor recorded `applied` against a human-approval-only irreversible verb. **The definitions here are IDENTICAL** — the row's own first narrowing (*this door already refuses `definitions_diverge`*) held only for the lens's fixture, and **[Observed]** the same collapse MERGES at `definitions_similarity:1.0000`. *A narrowing that holds for one fixture is not a narrowing.* Non-overridable | governance identity |
| C19-98 | **A3 at the door the THIRD trip came through** *(row 6d; **R91**)*. `retire(successor=)` redirects the old word at confidence 1.0 — §5.3's guarantee — so a successor whose declarations contradict makes one word answer two ways. **`force` included**: it overrides what could be SEEN, never what would become TRUE | governance identity |
| C19-99 | **the narrowing beside `C19-97`.** The rule is about CONTRADICTORY governance, not about actions being unmergeable — `ACTIONS.md` §2.1 argues the opposite and that argument stands. Two families whose declarations AGREE still merge — **the half a careless fix deletes, and it stands.** **SUPERSEDED by founder ruling R102 (`Q99`, `all eight`, row 6g), kept struck rather than deleted:** *"the four keys deliberately not compared (`inputs`, `preconditions`, `reachability`, `payload_schema`) are the family's SHAPE: a shape divergence does not let a caller invoke something they could not invoke before."* **That claim was never exercised by this id's body** — both its families are identical on all eight keys — and it is **wrong on its own terms**: a `preconditions` divergence DOES let a caller invoke something they could not invoke before, because the survivor's guard stops applying to the word the caller still uses (`C19-104`) | narrowing |
| C19-100 | **findings A1 and A2 — the defect this row MADE, removed** *(row 6d, round 3; ruling **R94**)*. `304967a`, this row's own A3 fix, compared every governance key with `!=`. Three of the four are scalars, where that is right; **`effects` is a LIST**, so two families whose governance is IDENTICAL but whose effects are written in a different order were refused `action_declarations_diverge` — **non-overridably, at all three doors, under every acknowledgement and under `force=True`**, on sqlite and postgres. **It closed a legal operation.** `ACTIONS.md` settles it three times: §2.5 defines effect identity as `(op, namespace, family, kind)`, §3.3's mechanism is set containment, §1's non-goals say *no ordering*. The comparison now uses the package's own `effect_identity`, which `record_invocation` had been using correctly 2,000 lines away — **one fact, one home** | **A1** |
| C19-101 | **the same expression's other half** *(round 3)*. §2.5: *"`why` is not part of identity for the three protocol ops, so amending a sentence does not turn one declared effect into two."* Under `!=` a single full stop closed the merge permanently | **A2** |
| C19-102 | **the narrowing this fix must not delete**, stated by §2.5 in the same breath: *"`host_state` has no target at all, so its `why` IS its identity"* (contortion **ACT9**, ruling **R46**). Two different admissions are two effects and still refuse. A set keyed on `effect_identity` gets this free; a hand-rolled sort would have had to re-implement it, which is why the fix uses the shared function | narrowing |
| C19-103 | **an extra effect, and a differing `min_auto_tier`, still refuse** *(round 3)*. Here because **no id drove either key**: round 3's actions lens cut `_GOVERNANCE_KEYS` to two entries and the suite stayed **identical to baseline**, because all three A3 fixtures build `effects=[]` and `min_auto_tier=None` on BOTH sides. Two of the four keys were load-bearing for zero ids — *a fixture that cannot fail on its own subject*, which is why the mutation removing half the rule survived | narrowing |
| C19-104 | **founder ruling R102 — `Q99` ruled `all eight`** *(row 6g)*. `_GOVERNANCE_KEYS` compared FOUR of `ACTIONS.md` §2.2's EIGHT declared keys, and `preconditions` was not one of them, so two families agreeing on all four and differing on `preconditions` collapsed with **no refusal, no `force` and no acknowledgement** — `resolve_type` then answered the dead word with the survivor at **1.0**, a Haiku actor recorded `applied`, and the survivor's ledger read `n=0`. **That is the route by which `A3` fires the governance stop criterion** (R100). Asserted at `merge_types` and at `retire(successor=)` under `force=True` | **A3** |
| C19-105 | **`C19-100`'s defect, TRIPLED by R102 and refused in advance** *(row 6g)*. `all eight` adds three more **list-valued** keys — `inputs`, `preconditions`, `reachability` — so the `!=` that closed a legal operation once would close it three more times, at the door where it is non-overridable. **Each key gets its own pair, and the reason first published for that was WRONG.** This row claimed a combined fixture *"goes green while two comparisons are still `!=`"*. **It goes RED** -- one refusal is enough to fail one merge, so a fixture reversing all three at once fails whenever ANY of the three is still `!=`. Separate pairs give better failure LOCALISATION and nothing more. **Found by the round's second lens, which built the combined fixture and ran it**; the original sentence argued the opposite of what the code does, which is the `C19-99` shape committed by the row created to catch it. The door now shares the read side's `_UNORDERED_DECLARED_KEYS` rather than holding a second opinion | narrowing |
| C19-106 | **W5 — a key PRESENT on one side and ABSENT on the other, which R102 §4 deliberately does NOT rule** *(row 6g)*. *"`all eight` says which keys are compared when both sides declare, not what to do when one does not."* Pins the **least-refusing** answer on the four keys R102 added, so a later row cannot extend the `.get()` comparison onto them by side effect. **Both cases are asserted together and that is the point:** under `.get()`, `reachability=[]` and an absent `reachability` compare EQUAL (both read as the empty set) while `reachability=["mcp"]` against the same absence REFUSES — **[Observed]** from a working implementation. A comparator that reads an absence as agreement in one case and a contradiction in the other is not applying a rule. **Does NOT touch the four keys already compared**, which refuse on a per-key absence today and are ROUTED | narrowing |
| C19-107 | **the round's BLOCKING 1 and BLOCKING 3, in one id because they are one gap** *(row 6g, adversarial round)*. **BLOCKING 3:** `inputs` was load-bearing for **ZERO ids and ZERO gate cells** — dropping it from the write door left the contract suite **byte-identical to baseline**, which is `C19-103`'s own recorded shape re-created by R102's landing. **BLOCKING 1:** `inputs` and `preconditions` were compared through `_unordered`, which is `json.dumps` over the RAW STORED DICT, while `effects` went through `Effect.from_dict` + `effect_identity`, which NORMALISES — so two `InputSpec` dicts that parse to the **identical dataclass**, one omitting the optional keys whose value is `None`, were refused `action_declarations_diverge` **non-overridably at all three doors under `force`**, while the same omission on `effects` was PERMITTED. **Identical data, opposite answers, and the only difference was the comparison method** — `C19-100`'s sentence one key over, and `import_types` takes its attributes from exactly the producers that omit nulls. Both halves asserted together: the refusal alone would have gone green over the defect, the respelling alone would not have closed the coverage gap | **A3** |
| C19-108 | **the round's BLOCKING 2 — `ACTIONS.md` §1's *no ordering* does not stop at the first level of nesting** *(row 6g, adversarial round)*. `_unordered` unorders the TOP-LEVEL list; `InputSpec.kinds` is a list INSIDE a member and is read as pure membership (`kind not in spec.kinds`), never by position — so a pure element-ORDER flip of it was refused **non-overridably at all three doors**. `C19-105` cannot see this, because it reverses only the top level: **a defect one level down needs a fixture one level down.** The first reproduction attempt FAILED on `kinds=("entity","predicate")`, refused `input_kind_mismatch` at DECLARATION — the fixture defect this row's census made three times — and was retried with valid kinds rather than filed as evidence of absence | narrowing |
