...
| C19-99 | **the narrowing beside `C19-97`.** The rule is about CONTRADICTORY governance, not about actions being unmergeable — `ACTIONS.md` §2.1 argues the opposite and that argument stands. Two families whose declarations AGREE still merge, and the four keys deliberately **not** compared (`inputs`, `preconditions`, `reachability`, `payload_schema`) are the family's SHAPE: a shape divergence does not let a caller invoke something they could not invoke before | narrowing |
