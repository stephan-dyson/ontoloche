...
| 2.5-5 | A family declaring a `payload_schema` no schema answers to is **written**, with `payload_schema_unregistered:<name>`. The named schema is simply not in force, so the per-namespace `(namespace, "edge_payload")` schema still governs if there is one — and `attr_schema_version` is `None` only when **nothing** governs *(row 4c's first adversarial round: this rule said `None` unconditionally, which is false the moment a namespace-level schema exists, and false in the other direction too — under a namespace schema in `enforce` mode the write is refused rather than written)* | `C17-37` |
...
**So §5.4 is amended in this change.** The values row #4 added, with their carrier — the table below has **six** rows, because row 4b added `endpoint_type_merged` to it and the lead-in went on saying *"the five"*, and *"sixteen values across four carriers"* was row #4's count of a vocabulary that is now **twenty-seven across ten** *(both corrected by row 4c's first adversarial round, which found the miscount in the very section that exists to keep this vocabulary honest — and found it because row 4c had edited one row of this table for R38 without reading the sentence above it)*:
...
> **`edges` is ordered by `(at_depth, edge_id)`, and `reached` names the node each edge newly reached** — `None` for a self-loop, for a triangle's closing edge, and **for an endpoint already reached under another name of the same identity** *(the third case is row 4c's, and it is not a third meaning: it is the same rule applied to the IDENTITY rather than to the written name, which ruling R38 is what makes differ. A consumer following §9.3's worked example filters on `reached is not None` and therefore drops such an edge from the projection — correctly, because it reaches nobody new — and `endpoint_type_merged` / `edge_family_merged` are what say an identity was folded. Found by row 4c's second adversarial round, by the lens that builds beacon's slice 1.)* *(Both added by row 4b's third adversarial round, and §9.3's worked example is why. That example fills the Tenshen grounding bundle's `relations` slot from a depth-2 report — **the worked example for the reason the edge row exists** — and a reviewer implemented it the obvious way: compare each edge's endpoints against the ORIGIN. At depth 2 that is silently wrong, because the far end of a second-hop edge was never incident on the origin: `person#7` never appears, `task#77` appears twice, and there is no error, no warning and no `complete=False`. **Mechanism C, inside the example written to show a consumer how to avoid it.** Computing it correctly needs `edges` walked in discovery order against a growing visited set — an inference the walk can make once, exactly, and a consumer can only re-derive. The order is therefore a guarantee rather than an accident: a deterministic traversal order, **not a ranking** — §1's *"a set, not a ranked list"* is about relevance and stands. `C17-34`.)*
...
| 4.3-15 | The successor chain **cannot be followed to an end** — a chain longer than the cap, or a backend that cannot page its retired rows | The walk **stops** and says so: `complete=False` with a `why` naming what stopped it, and it never claims to have resolved an identity it did not finish resolving. `C0-10`'s question — *can a broken backend make this loop?* — asked of chain-following, and *"it hangs"* is not an answer. **A CYCLE is deliberately NOT in this row** *(corrected by row 4c's first adversarial round, which found this rule claiming it was)*: `retire(a, successor=b)` followed by `retire(b, successor=a)` is constructible — nothing in `INTERFACE.md` §5.9 forbids it — but the visited set resolves such a closure **completely**, so the walk searched every name the identity spans and `complete=True` is the true statement. A cycle is a strange vocabulary, not a truncated read. | `C17-45` |
| 4.3-16 | A named `edge_families` entry is joined to another **family** by a merge or a retirement-with-successor | **The walk searches the other name too**, and the report carries `edge_family_merged:<namespace>:<name>`; edges found under the absorbed name carry `via_successor`, exactly as an absorbed *endpoint* does. *(Ruling **R38** followed the chain for endpoint TYPES and not for family NAMES until row 4c's second adversarial round. §2.3's bet is that a family **is** a `TypeEntry` and inherits `merge_types` for free; what that inheritance did was orphan every edge written under an absorbed family name, under `complete=True`, with no warning — **the exact sentence R38 exists to close, one axis over, inside the row that closed it.** Every R38 test merged `entity` types.)* | `C17-51` |
| 4.3-17 | The walk's origin names a type that has been **retired** | The walk proceeds — its edges were not deleted, which is `4.3-3`'s argument one object along — and the report carries `origin_type_retired:<ref>`. *(Added by row 4c's second adversarial round: `4.3-3` covers a retired FAMILY and `4.3-10` an UNREGISTERED origin, and a deliberately retired origin type — mechanism **3** — had no carrier at all.)* | `C17-52` |
...
> **The value-level edge gate now has a home — ruling R60's one change, landed 2026-09-03 by row 7a.** **Q17**, ruled **R22**, deferred a gate over an edge's *payload values* to Phase 3 *"because it would make `Consumer.gate` a query language"*. Phase 3's spec row has now specified that language once for all four surfaces that were reaching for it: [`INGEST.md`](INGEST.md) §6 — a `Condition` of twelve closed terms over **one record's attribute values**, three-valued (holds / fails / **unknowable**), declared on the entry and evaluated by the registry. An edge payload is one such record, so nothing further is owed by this document's shape. **Nothing here changes yet**, and the sequence is deliberate: `INGEST.md` §9 reserves its ids as *planned*, and the build row that lands `Condition` is where `EDGES.md` gains a rule, an id and a printed field. Recorded now because R60 requires the four surfaces to be amended in one change, and a pointer nobody wrote is how a deferred decision becomes a forgotten one.
