...
| the concurrent-approval race (`test_concurrency.py`), one per backend | 2 |
| **total** | **267** |
...
| **D-A10** | Four test modules beyond the 109: `test_manifest.py`, `test_generated_matches_source.py`, `test_parity.py`, `test_concurrency.py`. All `nonbinding`; none claims a contract id. | Suite bookkeeping, the anti-drift check, surface parity, and the race G2 exists to prevent. Extends 2A's D-14. |
