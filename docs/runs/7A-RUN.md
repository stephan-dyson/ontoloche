...
|---|---|---|
| **1** | ~~**E22 / P1 — §6.9c's eight named rules have no red check**~~ — **CLOSED at `T1.13`.** The row obliged itself to this in round 2, did not do it in round 3's first fix, and has now done it: **[Observed]** design test 1 carries a paired check for **`C20-01`** (the rule the whole R78 verdict rests on — the mutation hands back `cms:entity:facility#minted-by-registry` and the check sees it), **`C20-04`** (the adapter surface is two reads; a `put_instance` reddens it), **`C20-58`** (`CandidateQuery` carries no tenant field), **`C20-25`** (`tier` echoed back), **`C20-18`** (`known` must agree with `len(candidates)`; the mutation reports 2 and returns 1) | Closed. Rule 2-12 was already red; **`C20-05` and `C20-08` remain open** and are named in §8.1 |
| **2** | **E3 / P6 — sixteen mutation survivors of 33** | **Three closed at `T1.13`:** **rule 2-14 / `C20-65`**, which §13's route 12 rests on and which had no check at all; **`C20-77`**, the hop cap, now asserted at the shipped value of 16; **`C20-78`**, the cycle guard, now proved by a 65-hop runaway with the guard lifted. **The rest remain open**, and a survivor count — not a red count — is the measure (E23) |
