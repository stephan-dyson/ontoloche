...
## 6. The adversarial loop — and it ran with three lenses where the brief asked for six

**Stop rule** (standing constraint 7, and the brief's): two consecutive clean rounds, or three rounds plus an honest convergence note.
...
| **2** | fix-auditor on round 1's own fixes · *(kill-row lens did not report)* | **3** | 3 | 2 | **1** (the tenth) | 322 |
| **3** | — | — | — | — | — | — |

**Round 2 ran with ONE lens, and that is a gap rather than a result.** The fix-auditor lens completed and produced everything in §3 and §5.3. The **kill-row lens — briefed with all nine trip records and pointed at the new `ACTIONS` doors — terminated on a weekly API rate limit** after reporting only *"two failures appeared on the Postgres leg"*, with no report. **Its questions are unasked**, and this row does not get to count them as answered. Row 6b's own §7 lesson applies to itself: a checker only asks the questions its fixtures can pose, and a loop only answers the questions its lenses got to ask.

### 6.1 What the loop established that no gate here measures

1. **Both trips were in a FIX, not in the original code.** The ninth was in a fallback that shipped in row 4c and that this row's extraction *made visible* and raised as a question before any reviewer read it. The tenth was in the fix for the ninth, one branch along, **found by the lens whose whole job is round 1's fixes**. Row 4d measured this exact pattern (five defects inside round 1's fixes) and this row reproduced it at the same rate.
2. **Three BLOCKING in round 2 are one sentence: a fix applied at one call site out of two.** Refusal #1's computed operand on one branch; the Rule-U sentence wired into `invocations()` and dropped in `review_invocation`; the schema looked up by the wrong one of two names. That is the **eighth** trip's own diagnosis — *a fix that publishes a shared answer is only as good as its application* — arriving three times in one round.
3. **The integrator lens found nothing a correctness lens did, for the third row running**, and all three of its BLOCKING were in the **read** half or in *what the code does when nobody passed the optional argument*. The write half had been reviewed three times as a specification; the read half had never been used.
4. **Two defects were found by GATES rather than by reviewers**, both inside the change that introduced them: `check_capability_matrix.py` caught an asymmetric comparison manufacturing a difference on a degraded backend, and `C12-15`'s narrowing caught a `None` where a guard needed skipping. Constraint 8's gates and constraint 7's loop each found what the other could not, for the fifth consecutive row.
5. **The findings did NOT shrink**, and the reason is not that the work is bad: round 2 had one lens where round 1 had two, so the count is not comparable. **What is comparable is that the round pointed at yesterday's fixes found a kill-row trip on its first target.**

### 6.2 What a third round would find, [Assumed]

Every round of this row attacked something the previous round wrote and succeeded. **Round 2's fixes have had no such round, and the kill-row lens's questions were never asked at all.** The most likely next finding, on this row's own evidence: **a fourth call site of one of round 2's three one-site fixes**, or the first walk through the `ACTIONS` doors that no lens has yet driven — `preflight`'s precondition evaluation reads a predicate's extent, which makes it a **sixth** place that can disagree with the guards about what a word means, and that is the seventh trip's shape exactly.

**Stopping here is not the cap and not a verdict that the work is clean.** It is a rename landing between rounds; §6's table has an empty row and this section is the record that it is empty.
