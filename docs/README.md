...
| [PACKAGE.md](https://github.com/stephan-dyson/open-ontology/blob/main/docs/specs/PACKAGE.md) | v0, unstable | The `open_ontology` package: a fifteen-primitive storage-adapter protocol, SQLite + Postgres backends, `attributes` schema-per-kind, and the **113-test contract suite that defines conformance** (the Phase 2B gate; 109 at #3, four added by row 3c — two cross-namespace, one racing G1/G2, one on the per-kind attribute-schema limit). |
